...
async function generateImage(prompt){
  ...
  return {data:`data:image/png;base64,${base64}`,buffer:Buffer.from(base64,'base64')}
}
...
export async function buildSermonSlides(notes,source,onProgress=()=>{}){
  ...
  let completed=0
  const generated=await mapLimited(specs,3,async(spec,index)=>{const image=await generateImage(spec.visual);completed+=1;await onProgress({completed,total:specs.length});return {...image,index:index+1,title:clean(spec.title)||`Section ${index+1}`,kind:spec.kind,visual:spec.visual}})
  ...
  return {buffer:await pptx.write({outputType:'nodebuffer'}),slideCount:specs.length,assets:generated.map(({data,...asset})=>asset)}
}
