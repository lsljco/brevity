...
const schema={type:'object',additionalProperties:false,required:['meals','warnings'],properties:{
  meals:{type:'array',items:{type:'object',additionalProperties:false,required:['name','ingredients','calories','proteinGrams','carbohydrateGrams','fatGrams','serving','warnings'],properties:{
    name:{type:'string'},ingredients:{type:'array',items:{type:'string'}},calories:{type:['number','null']},proteinGrams:{type:['number','null']},carbohydrateGrams:{type:['number','null']},fatGrams:{type:['number','null']},serving:{type:'string'},warnings:{type:'array',items:{type:'string'}},
...
export async function analyzeMealImage(body,{fetcher=globalThis.fetch}={}){
  ...
  let response
  try{response=await fetcher('https://api.openai.com/v1/responses',{method:'POST',signal:controller.signal,headers:{authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'content-type':'application/json'},body:JSON.stringify({model:MODEL,store:false,max_output_tokens:2200,instructions:'Read a meal graphic carefully. Extract each distinct plated meal as a separate draft. Copy meal names, ingredient names and ALL FOUR per-plate macro figures only when actually printed. Do not infer portions, cooking instructions, preparation time, hidden ingredients or nutrition from the photograph. Return null for any missing or illegible macro number and explain uncertainty in warnings. An ingredient shown by name without amount stays just its name. Ignore marketing claims. Never silently combine meals.',input:[{role:'user',content:[{type:'input_text',text:'Extract up to 10 separate meals and their explicitly displayed calories, protein, carbohydrates and fat. Each draft will require household review before saving.'},{type:'input_image',image_url:`data:${image.mimeType};base64,${image.imageBase64}`}]}],text:{format:{type:'json_schema',name:'meal_image_import',strict:true,schema}}})})}catch(error){if(error.name==='AbortError')throw fail('Image analysis timed out. Try again.',504);throw error}finally{clearTimeout(timer)}
