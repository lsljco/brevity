...
export default async function handler(request){
  ...
  if(!session)return json(401,{error:'Sign in to check workout-image generation.'})
  ...
  const store=getStore({name:FITNESS_IMAGE_JOB_STORE,consistency:'strong',siteID:process.env.NETLIFY_SITE_ID,token:process.env.NETLIFY_TOKEN})
  const job=await store.get(fitnessImageJobKey(householdId,jobId),{type:'json'}).catch(()=>null)
