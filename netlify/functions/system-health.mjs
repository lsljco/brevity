...
export async function buildSystemHealth(event={}){
  ...
  try{const repository=await getOneDriveRepositoryState();checks.oneDrive=!oneDriveConfigured()?status('needs-attention','OneDrive publishing is not configured; connection changes are disabled in this release.'):repository.connection?status('ready',`Connected to ${repository.connection.account||'Microsoft account'}.`):repository.changeRequired?status('needs-attention','The stored OneDrive authorization targets an outdated repository; connection changes are disabled in this release.'):status('needs-attention','No existing OneDrive authorization is available; connection changes are disabled in this release.')}catch(error){checks.oneDrive=status('needs-attention',error.message||'OneDrive state could not be read.')}
