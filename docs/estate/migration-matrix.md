...
| Maintenance list | LocalStorage maintenance array | HomeHQ Maintenance | IMPROVE | Estate Maintenance + HomeHQ | Transform into WorkOrders linked to systems/assets; preserve shared project architecture | Status mapping, counts, idempotency | In progress |
| Preventive maintenance | Generic maintenance items | Generic HomeHQ item | ADD | Estate Maintenance | Durable plans generate related events/work orders; strict lifecycle records cost before next recurrence; explicit Family Calendar projection | Recurrence, lifecycle, concurrency and calendar de-duplication tests | In progress |
...
| Calendar | Browser events + CalDAV | Family Calendar | REPLACE | Family Calendar | Selectively reconcile; preventive maintenance now projects through Brevity auth and stable source IDs | Duplicate, timezone and CRUD tests | In progress |
