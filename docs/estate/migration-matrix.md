...
| Maintenance history | Overwritten current records | Notes/project history only | ADD | Estate Maintenance | Preserve completed events and source metadata | Historical count/field reconciliation | Not started |
...
| Document Vault | Mostly static categories/local files | HomeHQ file fields, sermon document stores | ADD | Estate Vault | Typed documents related to any Estate entity; binary store | MIME, permission, hash and relationship tests | Not started |
| Photos | Embedded data URLs | HomeHQ embedded files | REPLACE | Estate Documents | Extract binaries; object/blob storage and thumbnails | File-count/hash/render validation | Not started |
...
| Settings backup export | Prefix-based localStorage JSON | Browser export + server stores | REPLACE | Settings / Estate admin | Inspect exports from every device; manifest keys/files/hashes; preserve originals | Duplicate, checksum and no-mutation tests | In progress |
