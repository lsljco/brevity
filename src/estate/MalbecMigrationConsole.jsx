...
import { previewMalbecBackup } from './estateApi.js'
...
export default function MalbecMigrationConsole({ role, workspace }) {
  ...
  const resetValidation = () => { setPreview(null) }
  ...
  return <section className="estate-migration-console">
    ...
    {workspace && pendingFiles.length > 0 && <div className="estate-vault-import">
      ...
      {sourceMatchesWorkspace && <p>Matching file payloads can be inspected locally, but this release will not upload them or change the Estate workspace.</p>}
      <button type="button" disabled title="Estate Vault imports require Action Mode review, audit history, safe Undo, and version-conflict recovery.">Estate Vault import unavailable</button>
    </div>}
    {preview && <div className={`estate-preview-result${previewBlocks.length ? ' has-blockers' : ''}`}>
      ...
      {!workspace && <p className="estate-import-confirm"><span>Creating an Estate workspace is unavailable until the import can use Action Mode review, an immutable audit trail, safe Undo, and exact version-conflict protection.</span></p>}
      <button className="estate-commit-button" type="button" disabled title="Estate imports are preview-only in this release.">{workspace ? 'Initial import already protected' : 'Estate import unavailable'}</button>
    </div>}
  </section>
}
