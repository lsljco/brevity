...
export function buildAssistantContext({storage,member,activeView,activePillar,pageLabel,query,now=new Date()}){
  ...
  return{generatedAt:now.toISOString(),signedInMember:member,currentPage:{view:activeView,pillar:activePillar||null,label:pageLabel||activeView},todayPillarAnalyses:currentPillarAnalyses(storage,date,member),publicHealthAlerts:read(storage,STORAGE_KEYS.healthAlerts,null),finance:{plan:read(storage,STORAGE_KEYS.financePlan,{}),transactionSummary:summarizeTransactions(transactions),relevantTransactions:selectRelevantTransactions(transactions,query),budgets:read(storage,STORAGE_KEYS.budgets,{}),actualOverrides:read(storage,STORAGE_KEYS.actualOverrides,{}),transactionOverrides:read(storage,STORAGE_KEYS.transactionOverrides,{}),transactionRules:read(storage,STORAGE_KEYS.transactionRules,[]),categories:read(storage,STORAGE_KEYS.categories,[]),scenarios:read(storage,STORAGE_KEYS.scenarios,[]),goals:read(storage,STORAGE_KEYS.goals,[]),dailyAlignment:read(storage,STORAGE_KEYS.dailyAlignment,{})},projects:boundedArray(read(storage,STORAGE_KEYS.projects,[]),250),calendars:{brevityEvents:boundedArray(read(storage,STORAGE_KEYS.familyEvents,[]),300),appleFamilyCalendar:boundedArray(read(storage,STORAGE_KEYS.iCloudCalendar,null),300)}}
}
