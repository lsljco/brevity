...
function SettingsPage({ currentMember, role }) {
  ...
  return <div className="settings-page" style={{maxWidth:820,margin:'0 auto',padding:'48px 32px'}}>
    ...
    <div style={card}><div className="settings-card-row" style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:20}}><div><p style={title}>Local Recovery Cache</p><p style={sub}>Brevity's authenticated household service is authoritative for synchronized household records. This export is a local recovery copy of selected Finance, HomeHQ, calendar, and household-operation caches from this browser; restoring it republishes eligible shared records through household synchronization.</p></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button onClick={handleExport} style={{padding:'9px 18px',borderRadius:10,background:'rgba(197,164,109,.1)',border:'1px solid rgba(197,164,109,.25)',color:'var(--gold)',fontSize:13,cursor:'pointer'}}>Export Recovery Cache</button><button onClick={()=>importRef.current?.click()} style={{padding:'9px 18px',borderRadius:10,background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.12)',color:'var(--soft-white)',fontSize:13,cursor:'pointer'}}>Restore Recovery Cache</button><input ref={importRef} type="file" accept="application/json,.json" onChange={handleImport} style={{display:'none'}}/></div></div></div>
    <SermonDeviceRescue/>
