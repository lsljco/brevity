...
function PillarPlaceholder({ pillar }) {
  return <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'100%',gap:24,padding:40,textAlign:'center'}}><div style={{width:96,height:96,borderRadius:'50%',background:'linear-gradient(145deg, rgba(197,164,109,0.12), rgba(197,164,109,0.04))',border:'1.5px solid rgba(197,164,109,0.22)',display:'flex',alignItems:'center',justifyContent:'center'}}><i className={`ti ${pillar.icon}`} style={{fontSize:42,color:'var(--gold)'}} /></div><div><h2 style={{margin:'0 0 12px',fontFamily:'var(--font-serif)',fontSize:34,fontWeight:400,color:'var(--white)'}}>{pillar.label}</h2><p style={{margin:'0 0 28px',color:'var(--muted)',fontSize:14,maxWidth:400,lineHeight:1.7}}>{pillar.description}</p><span style={{display:'inline-block',padding:'7px 22px',borderRadius:24,border:'1px solid rgba(197,164,109,0.20)',background:'rgba(197,164,109,0.06)',fontSize:11,color:'rgba(197,164,109,0.65)',letterSpacing:'.12em',textTransform:'uppercase'}}>Coming Soon</span></div></div>
}
...
export default function App() {
  ...
  const openPillar=pillarId=>{setActivePillar(pillarId);setActiveView('__placeholder__');setExpandedPillar(pillarId)}
  const handlePillarClick=pillar=>pillar.items.length===0?openPillar(pillar.id):setExpandedPillar(prev=>prev===pillar.id?null:pillar.id)
  ...
  const renderContent=()=>{
    ...
    const pillar=PILLARS.find(p=>p.id===activePillar);return pillar?<PillarPlaceholder pillar={pillar}/>:null
  }
