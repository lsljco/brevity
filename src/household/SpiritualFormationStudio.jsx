...
import { archiveSermonDocuments, generateSermonFormation, generateSermonSlides, getOneDriveStatus, getSermonSlideStatus, importSermonNotes, ONEDRIVE_REPOSITORY_SHARE_URL, oneDriveConnectUrl } from './sermonFormationApi.js'
...
export default function SpiritualFormationStudio({draft,update}){
  ...
  const authorizeRepository=()=>window.location.assign(oneDriveConnectUrl(ONEDRIVE_REPOSITORY_SHARE_URL))
  ...
  return <div className="spiritual-studio">
    ...
    {hasGenerated&&<>
      ...
      <section className="sermon-document-actions">
        <div><span>Document Repository</span><strong>Sermon Documents &amp; Media</strong>{archivedDocument?.oneDrive?.state==='published'?<small className="sermon-cloud-ready"><i className="ti ti-cloud-check"/> Latest Word and PDF files published to Church Triumphant OneDrive.</small>:archivedDocument?<small className="sermon-cloud-ready"><i className="ti ti-device-floppy"/> Word and PDF saved in Brevity.</small>:null}{archivedDocument?.oneDrive?.devotions?.state==='published'&&<small className="sermon-cloud-ready"><i className="ti ti-cloud-check"/> Seven-Day Devotions published to Devotions.</small>}{archivedDocument?.oneDrive?.devotions?.state==='error'&&<small className="sermon-library-error">Seven-Day Devotions could not publish: {archivedDocument.oneDrive.devotions.error}</small>}{oneDrive.connected?<small className="sermon-cloud-ready"><i className="ti ti-cloud-check"/> Church Triumphant repository connected{oneDrive.connection?.account?` · ${oneDrive.connection.account}`:''}. Daily devotion PDFs publish to Devotions.</small>:oneDrive.configured&&!oneDrive.loading?<small className="sermon-cloud-ready"><i className="ti ti-folder-check"/> Church Triumphant repository location configured.</small>:null}{slides.state==='generating'&&<small className="sermon-cloud-ready"><i className="ti ti-photo"/> Creating photorealistic sermon slides{slides.total?` · ${slides.completed||0} of ${slides.total} images`:''}…</small>}{slides.oneDrive?.state==='published'&&<small className="sermon-cloud-ready"><i className="ti ti-cloud-check"/> PowerPoint published to Sermon Notes.</small>}{slides.oneDrive?.state==='error'&&<small>PowerPoint saved in Brevity, but OneDrive reported: {slides.oneDrive.error}</small>}{slides.state==='error'&&<small>{slides.error}</small>}{archiveError&&<small>{archiveError}</small>}</div>
        <div>{archivedDocument&&<><a href={archivedDocument.files.docx}><i className="ti ti-file-type-docx"/> Word</a><a href={archivedDocument.files.pdf}><i className="ti ti-file-type-pdf"/> PDF</a></>}{slides.state==='ready'&&<a href={slides.download}><i className="ti ti-file-type-ppt"/> PowerPoint</a>}{hasGenerated&&slides.state!=='ready'&&<button type="button" disabled={slides.state==='generating'} onClick={createSlides}><i className={`ti ${slides.state==='generating'?'ti-loader-2':'ti-presentation'}`}/> {slides.state==='generating'?'Creating slides…':'Create sermon slides'}</button>}{oneDrive.configured&&!oneDrive.connected&&!oneDrive.loading&&<button type="button" onClick={authorizeRepository}><i className="ti ti-brand-microsoft"/> Authorize publishing</button>}<a href={ONEDRIVE_REPOSITORY_SHARE_URL} target="_blank" rel="noreferrer"><i className="ti ti-brand-onedrive"/> Open repository</a>{archivedDocument?.oneDrive?.folderWebUrl&&<a href={archivedDocument.oneDrive.folderWebUrl} target="_blank" rel="noreferrer"><i className="ti ti-folder-open"/> Open Sermon Notes</a>}<button type="button" disabled={archiveState==='saving'} onClick={archiveCurrent}><i className={`ti ${archiveState==='saving'?'ti-loader-2':'ti-device-floppy'}`}/> {archiveState==='saving'?'Saving…':archivedDocument?'Update documents':'Save documents'}</button></div>
      </section>
