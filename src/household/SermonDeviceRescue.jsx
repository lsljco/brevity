...
export default function SermonDeviceRescue(){
  ...
  useEffect(()=>{let live=true;getSermonDeviceRescues().then(value=>{if(live){setData(value);setState('ready')}}).catch(reason=>{if(live){setError(reason.message);setState('error')}});return()=>{live=false}},[])
  ...
  return <section className="sermon-rescue-settings">
    ...
    <div className="sermon-rescue-metrics"><div><strong>{data.imports.length}</strong><span>Device exports preserved</span></div><div><strong>{data.sermons.length}</strong><span>Unique sermon versions</span></div><div><strong>{conflicts}</strong><span>Versions needing review</span></div></div>
    {data.imports.length>0&&<div className="sermon-rescue-imports"><h3>Immutable device backups</h3>{data.imports.map(item=><article key={item.id}><div><strong>{item.deviceLabel}</strong><span>{item.counts.sermons} sermons · {item.counts.revelationThreads} threads · imported by {item.importedBy}</span></div><a href={item.backupDownload}>Download backup</a></article>)}</div>}
