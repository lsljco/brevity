...
export function normalizeHouseholdMaintenanceState(value={}){const legacy=value.completions&&typeof value.completions==='object'?value.completions:{},occurrences=value.occurrences&&typeof value.occurrences==='object'?value.occurrences:legacy;return{version:4,trackingStartedOn:/^\d{4}-\d{2}-\d{2}$/.test(String(value.trackingStartedOn||''))?value.trackingStartedOn:maintenanceDateKey(maintenanceToday()),occurrences,completions:occurrences}}
...
export function householdOperationCalendarEvents(state,{start=maintenanceWeekStart(maintenanceToday()),weeks=6}={}){const tasks=[];const first=maintenanceWeekStart(start);for(let index=0;index<Math.max(1,Math.min(54,Number(weeks)||6));index+=1){const anchor=addDays(first,index*7);buildHouseholdMaintenanceWeek(anchor).forEach(day=>day.tasks.forEach(item=>{if(item.calendarEnabled)tasks.push(item)}))}return tasks.map(item=>householdOperationCalendarEvent(item,householdOccurrence(normalizeHouseholdMaintenanceState(state),item)))}
