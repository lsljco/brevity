...
export function normalizeHouseholdMaintenanceState(value={}){const legacy=value.completions&&typeof value.completions==='object'?value.completions:{},occurrences=value.occurrences&&typeof value.occurrences==='object'?value.occurrences:legacy;return{version:4,trackingStartedOn:/^\d{4}-\d{2}-\d{2}$/.test(String(value.trackingStartedOn||''))?value.trackingStartedOn:maintenanceDateKey(new Date()),occurrences,completions:occurrences}}
...
export function occurrenceStatus(task,occurrence={}){if(occurrence.returnedAt)return'Returned';if(occurrence.approvedAt)return'Approved';if(occurrence.submittedAt)return'Awaiting sign-off';if(occurrence.exception)return'Exception';if(occurrence.complete&&!task.signoffRequired)return'Complete';return'Scheduled'}
export function summarizeHouseholdMaintenance(days,state,today=new Date()){const todayKey=maintenanceDateKey(today),tasks=days.flatMap(day=>day.tasks),approved=tasks.filter(task=>{const occ=householdOccurrence(state,task);return task.signoffRequired?Boolean(occ.approvedAt):Boolean(occ.complete)}).length,awaitingSignoff=tasks.filter(task=>{const occ=householdOccurrence(state,task);return Boolean(occ.submittedAt&&!occ.approvedAt&&!occ.returnedAt)}).length,exceptions=tasks.filter(task=>householdOccurrence(state,task).exception).length,covered=tasks.filter(task=>householdOccurrence(state,task).coveredBy).length,overdue=tasks.filter(task=>{const taskDate=task.occurrenceId.slice(0,10),occ=householdOccurrence(state,task),closed=task.signoffRequired?Boolean(occ.approvedAt):Boolean(occ.complete);return taskDate>=state.trackingStartedOn&&taskDate<todayKey&&!closed}).length,dueToday=tasks.filter(task=>task.occurrenceId.startsWith(todayKey)).length;return{scheduled:tasks.length,completed:approved,approved,awaitingSignoff,overdue,dueToday,exceptions,covered}}
