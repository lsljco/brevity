...
test('slide jobs are bound to the reviewed active artifact and never publish externally',async()=>{
  ...
  assert.equal(store.values.get(`lslj-family/slides/${id}/status`).oneDrive.state,'disabled')
...
test('the Spiritual Maturity UI preserves the active sermon while a candidate awaits Action Mode review',async()=>{
