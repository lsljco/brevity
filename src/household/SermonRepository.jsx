...
import { archiveSermonDocuments, getOneDriveStatus, getSermonDeviceRescues, listSermonDocuments, ONEDRIVE_REPOSITORY_SHARE_URL } from './sermonFormationApi.js'
...
export default function SermonRepository({notes,source}){
  ...
  const activeVersion=Number(source?.activeVersion||0)
  const sourceHash=String(source?.sourceHash||'')
  const reviewedActive=Boolean(notes)&&Number.isInteger(activeVersion)&&activeVersion>=1&&/^[a-f0-9]{64}$/.test(sourceHash)
  ...
  const createCurrent=async()=>{
    ...
    }catch(err){setPublishState('error');setPublishMessage(err.message||'Could not create the sermon documents.')}
  }
  return <section className="pillar-analysis-section sermon-repository">
    ...
    <div className="sermon-cloud-panel">
      <div><span>Church Triumphant OneDrive</span><strong>External publishing unavailable</strong><small><i className="ti ti-lock"/> Brevity creates files locally from the exact reviewed active sermon. It does not upload or replace external documents until publishing has its own reviewed, reversible workflow.</small>{oneDrive.connected&&<small>Existing repository connection detected{oneDrive.connection?.account?` · ${oneDrive.connection.account}`:''}; it is not used for automatic publishing.</small>}{oneDrive.error&&<small className="sermon-library-error">{oneDrive.error}</small>}{publishMessage&&<small className={publishState==='error'?'sermon-library-error':''}>{publishMessage}</small>}</div>
      <div><button type="button" className="sermon-connect-cloud" disabled title="External publishing and connection changes are disabled in this release."><i className="ti ti-lock"/> External publishing unavailable</button>{oneDrive.connected&&oneDrive.connection?.folderWebUrl?<a href={oneDrive.connection.folderWebUrl} target="_blank" rel="noreferrer"><i className="ti ti-brand-onedrive"/> Open folder</a>:<a href={ONEDRIVE_REPOSITORY_SHARE_URL} target="_blank" rel="noreferrer"><i className="ti ti-brand-onedrive"/> View repository</a>}{notes&&<button type="button" disabled={publishState==='saving'||!reviewedActive} onClick={createCurrent}><i className={`ti ${publishState==='saving'?'ti-loader-2':'ti-device-floppy'}`}/> {publishState==='saving'?'Creating…':'Create local files'}</button>}</div>
    </div>
    {source?.document&&<div className="sermon-current-downloads"><strong>{source.document.title}</strong><span>{source.document.files?.docx&&<a href={source.document.files.docx}>Download Word</a>}{source.document.files?.pdf&&<a href={source.document.files.pdf}>Download PDF</a>}</span></div>}
    ...
    <div className="sermon-library-list"><h3>Repository Files</h3>{state==='loading'&&<p>Loading documents…</p>}{error&&<p className="sermon-library-error">{error}</p>}{state==='ready'&&!documents.length&&<p>No archived documents yet.</p>}{documents.map(document=><article key={document.id}><div><strong>{document.title}</strong><small>{[document.serviceType,document.sermonDate,document.preacherTeacher].filter(Boolean).join(' · ')}</small>{document.oneDrive?.state==='published'&&<small className="sermon-cloud-ready"><i className="ti ti-cloud-check"/> Previously published to OneDrive</small>}</div><span>{document.files?.docx&&<a href={document.files.docx}>Word</a>}{document.files?.pdf&&<a href={document.files.pdf}>PDF</a>}</span></article>)}</div>
