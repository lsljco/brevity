...
export default function SermonRepository({notes,source}){
  ...
  const publishCurrent=async()=>{
    ...
    try{
      ...
      if(document.oneDrive?.state==='published')setPublishMessage(`Published Word and PDF to ${document.oneDrive.folderName||'Church Triumphant OneDrive'}.`)
  ...
  return <section className="pillar-analysis-section sermon-repository">
    ...
    <div className="sermon-cloud-panel">
      <div><span>Church Triumphant OneDrive</span><strong>{oneDrive.loading?'Checking connection…':oneDrive.connected?`Connected to ${oneDrive.connection?.folderName||'Sermon Notes'}`:'Connect the shared Sermon Notes folder'}</strong>{oneDrive.connected&&<small>{oneDrive.connection?.account}</small>}{oneDrive.error&&<small className="sermon-library-error">{oneDrive.error}</small>}{publishMessage&&<small className={publishState==='error'?'sermon-library-error':''}>{publishMessage}</small>}</div>
