...
export default function SermonRepository({notes,source}){
  ...
  return <section className="pillar-analysis-section sermon-repository">
    ...
    <div className="sermon-cloud-panel">
      <div><span>Church Triumphant OneDrive</span><strong>{oneDrive.loading?'Checking OneDrive…':oneDrive.connected?'OneDrive connected':'OneDrive repository'}</strong>{oneDrive.connected&&oneDrive.connection?.account&&<small className="sermon-cloud-ready"><i className="ti ti-cloud-check"/> Connected as {oneDrive.connection.account}</small>}<small><i className="ti ti-lock"/> The reviewed repository folder is available below. Automatic uploads are not enabled; Create local files produces Word and PDF files for review without changing OneDrive.</small>{oneDrive.error&&<small className="sermon-library-error">{oneDrive.error}</small>}{publishMessage&&<small className={publishState==='error'?'sermon-library-error':''}>{publishMessage}</small>}</div>
      <div><span className="sermon-cloud-status"><i className="ti ti-lock"/> Automatic uploads not enabled</span>{oneDrive.connected&&oneDrive.connection?.folderWebUrl?<a href={oneDrive.connection.folderWebUrl} target="_blank" rel="noreferrer"><i className="ti ti-brand-onedrive"/> Open folder</a>:<a href={ONEDRIVE_REPOSITORY_SHARE_URL} target="_blank" rel="noreferrer"><i className="ti ti-brand-onedrive"/> View repository</a>}{notes&&<button type="button" disabled={publishState==='saving'||!reviewedActive} onClick={createCurrent}><i className={`ti ${publishState==='saving'?'ti-loader-2':'ti-device-floppy'}`}/> {publishState==='saving'?'Creating…':'Create local files'}</button>}</div>
    </div>
