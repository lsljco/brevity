...
function scheduleSnapshot() {
  try {
    getAcknowledgedSharedStateVersion(localStorage, HOUSEHOLD_SCHEDULE_STORAGE_KEY)
    return { data: normalizeHouseholdScheduleState(JSON.parse(localStorage.getItem(HOUSEHOLD_SCHEDULE_STORAGE_KEY) || '{}')), ready: true, error: '' }
  } catch { return { data: normalizeHouseholdScheduleState(), ready: false, error: '' } }
}
function usePracticeSchedule() {
  const [snapshot, setSnapshot] = useState(scheduleSnapshot)
  const [loading, setLoading] = useState(false)
  const mounted = useRef(true)
  const refresh = useCallback(() => setSnapshot(scheduleSnapshot()), [])
  const reload = useCallback(async () => {
    setLoading(true)
    try { await syncSharedState(); if (mounted.current) setSnapshot(scheduleSnapshot()) }
    catch (error) { if (mounted.current) setSnapshot(value => ({ ...value, ready: false, error: error.message || 'Schedule synchronization is unavailable.' })) }
    finally { if (mounted.current) setLoading(false) }
  }, [])
  useEffect(() => {
    mounted.current = true
    if (!scheduleSnapshot().ready) reload()
    const receive = event => {
    ...
    window.addEventListener(ACTION_COMPLETED_EVENT, receive)
    return () => { mounted.current = false; window.removeEventListener(SHARED_STATE_EVENT, receive); window.removeEventListener('storage', receive); window.removeEventListener(ACTION_COMPLETED_EVENT, receive) }
  }, [refresh, reload])
...
function WeeklyPracticeReview({ date, canEdit, onRecord, revision }) {
  ...
  useEffect(() => {
    ...
    Promise.all(dates.map(async day => {
      try { const plan = await fetchDailyPlan(day); return { date: day, ...readPracticeDay(plan), plan } }
...
export function OperatingPracticesPanel({ plan, planState = 'loading', planError = '', meals = {}, mealState = 'loading', currentMember, canEditPlanning = false, mode = 'daily', compact = false, onNavigate, onOpenPractices, onReload }) {
  ...
  useEffect(() => {
    const completed = event => {
      if (!pending.current || event.detail?.proposalId !== pending.current) return
  ...
  const open = (kind, details = {}) => { setError(''); setNotice(''); setEditor({ kind, ...details, plan: clone(plan) }) }
  ...
  const checkin = ({ status, note, recovery }) => stage(withPracticeCheckin(editor.plan, editor.card.practice, status, note, recovery))
  ...
  return <section className={`operating-practices${compact ? ' operating-practices--compact' : ''}`} aria-label="Household operating readiness" data-testid="operating-practices">
    ...
    {mode !== 'weekly' && <div className="practice-grid">{cards.map(card => <article key={card.template.id} className={`practice-card practice-card--${card.state}`}>
      ...
      {mode === 'policies' && <p>{card.template.purpose}</p>}<p>{card.template.standard}</p>
      {card.practice && <p className="practice-owner">{card.practice.routine.owner} · Backup: {card.practice.backup}<br />{card.practice.routine.startTime}–{card.practice.routine.endTime} · Agreement v{card.practice.revision}</p>}
    ...
    {mode === 'daily' && <>
      <section className="practice-meals"><header><div><h3>Meal coverage and availability</h3><p>Planning, preparation, communication and eating are different records.</p></div><button type="button" onClick={() => onNavigate?.('inventory')}>Check supplies &amp; inventory</button></header>
        {mealState !== 'ready' && <p role="status">The rolling meal source is not verified. Coverage below cannot confirm the current menu.</p>}
        <div className="practice-grid">{PRACTICE_MEALS.map(type => { const readiness = mealReadiness(mealState === 'ready' ? meals[type] : null, saved.data.meals[type]); const record = readiness.saved; return <article key={type} className={`practice-card practice-card--${readiness.state}`}>
          <span>{labelMeal(type)}</span><h4>{meals[type]?.name || 'Meal not available'}</h4><strong>{readiness.label}</strong>{record?.readyBy && <p>Ready by {record.readyBy} · {record.owner || 'Owner needed'} · {record.headcount ?? '?'} portions</p>}
          {record?.communicated && <p className="practice-availability">Available at: {record.location}</p>}{readiness.gaps.length > 0 && <p>{readiness.gaps.join(' · ')}</p>}
          {record?.portionsEaten != null && <p>{record.portionsEaten} portions reported eaten</p>}{record?.leftovers && !compact && <p>Leftovers / cleanup: {record.leftovers}</p>}
          <button type="button" disabled={!canEdit} onClick={() => open('meal', { type, meal: meals[type], saved: saved.data.meals[type] })}>Review {type} coverage</button>
        </article> })}</div>
      </section>
      ...
      <p className="practice-muted">Before leaving across a mealtime, confirm the meal or an approved funded alternative. This workspace does not authorize purchases, initiate bank transfers, send external notifications, or rate spiritual maturity.</p>
    </>}
    ...
    {editor?.kind === 'checkin' && <PracticeCheckinEditor card={editor.card} onReview={checkin} onClose={close} />}
...
export default function OperatingPracticesWorkspace({ currentMember, canEditPlanning = false, initialDate, initialTab = 'daily', onNavigate }) {
  ...
  const rolling = useRollingMealPlan({ reloadOnRefreshEvents: true })
  ...
  return <div className="practice-workspace"><header className="practice-workspace-header"><div><span>Household Management</span><h1>Policies &amp; Practices</h1><p>Daily finance · Household schedule · Meals and procurement</p></div><label><span>Household date</span><input type="date" value={date} onChange={event => { if (event.target.value) setDate(event.target.value) }} /></label></header>
    ...
    <OperatingPracticesPanel plan={daily.plan} planState={daily.state} planError={daily.error} meals={meals} mealState={rolling.state} currentMember={currentMember} canEditPlanning={canEditPlanning} mode={tab} onNavigate={onNavigate} onReload={daily.reload} />
  </div>
}
