...
export const maintenanceCompletionOperation = (task, action, reason='', completedItems=[]) => ({ type:'household.maintenance.completion.update',targetId:task.occurrenceId,targetDate:task.occurrenceId.slice(0,10),payload:{action,...(clean(reason)?{reason:clean(reason)}:{}),...(action==='submit'?{completedItems}: {})},description:`${action[0].toUpperCase()}${action.slice(1)} ${quote(task.title)} for ${task.occurrenceId.slice(0,10)}.` })
