  ...
  const calendarBatchSave = (txArray) => {
    const saved = setDataAndPersist(d => {
      ...
      txArray.forEach(tx => {
        ...
        if (idx >= 0) txns[idx] = tx; else txns.push(tx)
      })
...
function BudgetView({ data, plaidActuals = [], onOpenTransactions, onOpenRecurring }) {
  ...
  const itemActual = (item) => {
    const manual = getActual(item)
    if (manual !== '') return parseFloat(manual) || 0
    const needle = item.toLowerCase()
    return selectedMonthActuals.reduce((sum, tx) => {
      const name = String(tx.merchant_name || tx.merchant || tx.name || '').toLowerCase()
      return name && (name.includes(needle) || needle.includes(name)) ? sum + Math.abs(Number(tx.amount) || 0) : sum
    }, 0)
  }
  const catActual = (cat) => syncedActualsByCategory[cat]
    ?? (BUDGET_CATS[cat] || []).reduce((sum, item) => sum + itemActual(item), 0)
...
function CalendarView({ proj, calYear, calMonth, setCalYear, setCalMonth, selDay, setSelDay, accounts, viewAcctIds, onSave, onBatchSave, onApplyChanges, onDelete, showActuals, toggleActuals, actualsLoading, actualsError, actualsByDate, plaidActuals, historicalBals = {}, onSaveOverride, onRemoveOverride, onActualTxClick }) {
  ...
  const commitBal = (key, draft) => {
    ...
    if (!isNaN(n)) onSaveOverride(key, n)
    setEditBal(null)
  }

  // ── Move handlers ────────────────────────────────────────────────────────
  const handleMoveOne = ({ tx, fromDate, toDate }) => {
                ...
                <div style={{ marginTop: 'auto', paddingTop: 4, borderTop: '1px solid rgba(255,255,255,0.06)', textAlign: 'right' }}
                  ...
                  {isEditingBal ? (
                    <input autoFocus type="number"
                      value={editBal.draft}
                      onChange={e => setEditBal(b => ({ ...b, draft: e.target.value }))}
                      onKeyDown={e => { if (e.key === 'Enter') commitBal(key, editBal.draft); if (e.key === 'Escape') setEditBal(null) }}
                      onBlur={() => commitBal(key, editBal.draft)}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--muted)' }}>
                ...
                {selPt.txns.length > 0 ? ' · click to edit · drag to move' : ''}
              </p>
            </div>
