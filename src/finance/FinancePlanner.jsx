...
.cal-cell.drag-over {
  border-color: rgba(197,164,109,.85) !important;
  background: rgba(197,164,109,.12) !important;
...
.finance-form-grid,
.finance-summary-grid { min-width: 0; }
.finance-form-grid > *,
.finance-summary-grid > * { min-width: 0; }
.dash-actions .plaid-connect {
  width: 100%;
  min-width: 0;
  margin-bottom: 0 !important;
}
.plaid-connect-status,
.plaid-connect-disconnected { min-width: 0; max-width: 100%; }
...
@media (max-width: 768px) {
  ...
  .dash-actions > *:first-child {
    grid-column: 1 / -1 !important;
    width: 100% !important;
    transform: none !important;
  }
  .dash-search { grid-column: 1 / -1 !important; }
...
const DEFAULT_DATA = {
  ...
  accounts: [
    ...
    { id: 'a3', name: 'LSLJ Savings',            balance: 12000, type: 'savings',  plaidAccountId: null },
  ],
  transactions: [
    ...
    { id: 't_h6',    name: 'Property Taxes',                 amount: 30337.59, type: 'expense', freq: 'yearly',  start: '2026-07-14', end: '', cat: 'Housing',        acct: 'a2' },
...
function loadData() {
  try {
    ...
    return restorePersistedFinanceData(stored, DEFAULT_DATA)
...
function TxForm({ tx, accounts, onSave, onCancel }) {
  const blank = { name: '', amount: '', type: 'expense', freq: 'monthly', start: getHouseholdDateKey(), end: '', cat: 'Housing', acct: accounts[0]?.id || '', transferTo: '' }
  const [form, setForm] = useState(tx ? { ...tx } : blank)
  // Keep the version from the moment this editor opened. If household data
  // changes while the form is open, direct Action preparation must reject it
  // instead of treating stale form fields as intentional replacements.
  const [openedVersion] = useState(() => Object.hasOwn(tx || {}, '_reviewVersion')
    ? tx._reviewVersion
    : captureFinanceReviewVersion())
  ...
  const save = async () => {
    ...
    const saved = { ...form, amount: parseFloat(form.amount) || 0 }
    if (tx?.id) saved.id = tx.id
    saved._reviewVersion = openedVersion
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        <button aria-label="Back from transaction form" onClick={onCancel} style={{ padding: '6px 10px', cursor: 'pointer', borderRadius: 8, border: '1px solid rgba(0,0,0,0.12)', background: 'white', fontSize: 14 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 520 }}>
        ...
        <div className="finance-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label className="field-label">Type</label>
            <select value={form.type} onChange={e => set('type', e.target.value)} style={{ width: '100%' }}>
              <option value="income">Income</option>
              <option value="expense">Expense</option>
              <option value="transfer">Planned transfer (forecast only)</option>
            </select>
          </div>
          <div>
        ...
        {form.type === 'transfer' && (
          <div style={{ padding: '12px 14px', background: 'rgba(144,170,222,0.07)', border: '1px solid rgba(144,170,222,0.20)', borderRadius: 10 }}>
            ...
            <p style={{ margin: '8px 0 0', fontSize: 11, color: '#90AADE', opacity: 0.8 }}>
              Forecast only. This records a planned cash-flow transfer and cannot move money or change a bank account.
            </p>
          </div>
        )}
        <div>
        ...
        <div className="finance-form-grid" style={{ display: 'grid', gridTemplateColumns: form.type === 'transfer' ? '1fr' : '1fr 1fr', gap: 12 }}>
          {form.type !== 'transfer' && (
            <div>
              ...
              {customCatMode ? (
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <button
                    type="button"
                    aria-label="Use saved category list"
        ...
        <div style={{ display: 'flex', gap: 8, paddingTop: 4 }}>
          ...
          <button onClick={save} disabled={preparing} style={{ flex: 2, padding: '10px', cursor: preparing ? 'wait' : 'pointer', borderRadius: 10, border: 'none', background: '#C5A46D', color: 'white', fontWeight: 600, fontSize: 14, fontFamily: 'inherit', opacity:preparing ? .7 : 1 }}>
            {preparing ? 'Preparing review…' : tx?.id ? 'Review scheduled change' : 'Review scheduled transaction'}
          </button>
        </div>
      </div>
    </div>
...
export default function FinancePlanner({ view: extView, setView: setExtView, currentMember = 'Household member', readOnly = false, meetingPlanningReadOnly = readOnly }) {
  ...
  const [formView, setFormView] = useState(null) // 'tx-form' | null
  ...
  useEffect(() => {
    const receiveRefresh = event => {
      ...
      if (event.detail?.transactionFreshness) setActualsFreshness(event.detail.transactionFreshness)
      if (event.detail?.balanceDataStatus) setBalanceDataStatus(event.detail.balanceDataStatus)
      if (Array.isArray(event.detail?.balanceErrors)) setBalanceDataErrors(event.detail.balanceErrors)
    }
  ...
  useEffect(() => {
    const revalidateFreshness = () => {
      setActualsFreshness(readTransactionFreshness(localStorage))
      const balance = readLatestBalanceRefreshStatus()
      setBalanceDataStatus(balance.status)
      setBalanceDataErrors(balance.errors)
    }
    const revalidateWhenVisible = () => {
      if (document.visibilityState === 'visible') revalidateFreshness()
    }
    const interval = window.setInterval(revalidateFreshness, 60_000)
    window.addEventListener('focus', revalidateFreshness)
    document.addEventListener('visibilitychange', revalidateWhenVisible)
    return () => {
      window.clearInterval(interval)
      window.removeEventListener('focus', revalidateFreshness)
  ...
  const setView = (v) => {
    if (readOnly && v === 'tx-form') {
      setFormView(null)
      return
    }
    if (v === 'tx-form') { setFormView(v) }
    else { setFormView(null); setExtView?.(v) }
  }

  const [calYear, setCalYear]   = useState(() => getHouseholdCalendarDate().getFullYear())
  const [calMonth, setCalMonth] = useState(() => getHouseholdCalendarDate().getMonth())
  const _now = getHouseholdCalendarDate()
  ...
  useEffect(() => {
    const receiveSharedUpdate = event => {
      ...
      if (keys.has('lslj_finance_v9')) {
        ...
        setData(next)
        // A remotely applied balance snapshot has no local live-check proof.
        // Require this device to re-establish freshness before using it as a
        // current-liquidity or historical-reconstruction anchor.
        const balance = invalidateLatestBalanceRefreshStatus()
        setBalanceDataStatus(balance.status)
        setBalanceDataErrors(balance.errors)
      }
      if (keys.has('plaid_actuals_cache')) {
        setPlaidActuals(loadSavedValue('plaid_actuals_cache', null))
        // The local freshness record belongs to the device snapshot that was
        // previously rendered. A remote household snapshot has no bound local
        // check receipt, so do not present the old timestamp as proof of it.
        setActualsFreshness({ status:'unknown', checkedAt:'', lastFullSuccessAt:'', errors:[] })
      }
  ...
  const proj = useMemo(() => {
    ...
    // Manual balance overrides are intentionally ignored. The projection must
  ...
  const fetchActuals = useCallback(async () => {
    ...
    try {
      ...
      if (json.connected === false) {
        const message = 'No existing bank source is available. The last verified transaction history remains visible and is marked stale; bank connection changes are disabled in this release.'
        const freshness = recordTransactionFreshness(localStorage, { status:'stale', errors:[message], successfulInstitutions:[] })
      ...
      setActualsNotice(syncErrors.length
        ...
        : json.refresh?.stillProcessing
          ...
          : `Bank transactions checked at ${getHouseholdTimeLabel()}.`)
      return { ...json, transactions: txns }
    } catch (e) {
      const message='transactions could not be refreshed: '+e.message
      try { setActualsFreshness(recordTransactionFreshness(localStorage, { status:'stale', errors:[message], successfulInstitutions:[] })) } catch {}
      setActualsError(message)
      setActualsNotice('')
      return { error:message }
    }
    finally { setActualsLoading(false) }
  }, [plaidActuals, readOnly])
  ...
  const actualsFreshnessMessage = (() => {
    const checkedAt = actualsFreshness?.checkedAt ? getHouseholdDateTimeLabel(new Date(actualsFreshness.checkedAt)) : ''
    const lastFullSuccess = actualsFreshness?.lastFullSuccessAt ? getHouseholdDateTimeLabel(new Date(actualsFreshness.lastFullSuccessAt)) : ''
    if (actualsFreshness?.status === 'stale') return `Transaction history is stale${checkedAt ? ` as of the ${checkedAt} check` : ''}. Brevity is retaining the last verified snapshot${lastFullSuccess && lastFullSuccess !== checkedAt ? `; the last complete check was ${lastFullSuccess}` : ''}.`
    if (actualsFreshness?.status === 'partial') return `The Plaid transaction snapshot is partial${checkedAt ? ` as of ${checkedAt}` : ''}. Successful sources were updated and unconfirmed sources retain their last verified rows${lastFullSuccess ? `; the last complete check was ${lastFullSuccess}` : ''}.`
    if (actualsFreshness?.status === 'fresh') return `The latest Plaid transaction snapshot loaded without source errors${checkedAt ? ` at ${checkedAt}` : ''}. This is Brevity's check time, not a guarantee that every institution posted new activity at that time.`
    return plaidActuals?.length
      ? 'Transaction freshness has not yet been verified on this device; the last cached history remains visible.'
      : 'No verified bank transaction snapshot is available on this device. Scheduled entries remain projections and no historical close is reconstructed.'
  })()
  const balanceDataMessage = (() => {
    if (balanceDataStatus === 'fresh') return 'The latest balance attempt matched every returned bank account and completed without source errors.'
    if (balanceDataStatus === 'disconnected') return 'No connected bank balance source was confirmed. Stored balances remain visible but are not presented as current bank truth.'
    if (balanceDataStatus === 'partial') return `The balance attempt was partial. Confirmed linked balances were processed, while unconfirmed or unlinked accounts retain prior values. ${balanceDataErrors.join(' ')}`.trim()
    if (['ambiguous','incompatible','unmatched','unverified'].includes(balanceDataStatus)) return `The latest balance attempt did not establish a complete safe anchor. Prior balances were preserved. ${balanceDataErrors.join(' ')}`.trim()
    if (balanceDataStatus === 'stale') return `The latest balance attempt failed or is stale. The last safely stored balances remain visible. ${balanceDataErrors.join(' ')}`.trim()
    return 'Balance freshness has not yet been verified for this screen. Stored account values remain visible with their linkage provenance.'
  })()

  // Map plaid accountId → our local account id for filtering
  const plaidIdToLocal = useMemo(() => {
    return buildUniquePlaidAccountMap(data.accounts)
  }, [data.accounts])
  const linkedLocalAccountIds = useMemo(() => new Set(Object.values(plaidIdToLocal)), [plaidIdToLocal])
  const linkedForecastAccountCount = forecastScope.accounts.filter(account => linkedLocalAccountIds.has(account.id)).length
  const forecastHasLedgerAnchors = hasVerifiedCashLedgerAnchors(forecastScope.accounts, plaidIdToLocal)
  const forecastBalanceVerifiedLive = balanceDataStatus === 'fresh' && forecastHasLedgerAnchors
  const forecastCanReconstructHistory = forecastBalanceVerifiedLive && actualsFreshness?.status === 'fresh'
  const forecastBalanceSource = forecastBalanceVerifiedLive
    ? 'live bank balances confirmed for every included cash account'
    : forecastScope.accounts.length > 0 && linkedForecastAccountCount === forecastScope.accounts.length
      ? 'the latest stored values for linked cash accounts; current bank freshness is not proven'
  ...
  const actualAccountScope = useMemo(
    () => scopePlaidTransactionsByAccount(plaidActuals || [], plaidIdToLocal, activeAcctIds, {
      includeUnmapped:!selectedAccts || selectedAccts.size === data.accounts.length,
    }),
    [plaidActuals, plaidIdToLocal, activeAcctIds, selectedAccts, data.accounts.length],
  )
  const unmappedActuals = actualAccountScope.unmapped
  ...
  const transactionHistory = useMemo(() => Object.entries(transactionViewActuals.reduce((months, tx) => {
    const month = String(tx.date || '').slice(0, 7) || 'Unknown'
    if (!months[month]) months[month] = { count: 0, total: 0 }
  ...
  const transactionSummaryNet = showActuals
    ? transactionViewStats.income - transactionViewStats.expenses
    : scheduledViewStats.income - scheduledViewStats.expenses

  const openFilteredTransactions = (filter = null) => {
    setTransactionListOptions({ ...DEFAULT_TRANSACTION_LIST_OPTIONS })
    setTransactionFilter(filter)
    setShowActuals(true)
    setView('transactions')
  }
  const openAllBankActivity = () => {
    setSelectedAccts(null)
  ...
  const allTxNames = useMemo(() => {
    ...
    const names = new Set()
    plaidActuals.forEach(transaction => {
      const ruled = applyTransactionRules(transaction, txRules, data.accounts)
      const visible = txOverrides[transaction.id] ? { ...ruled, ...txOverrides[transaction.id] } : ruled
      ;[transaction.name, transaction.merchant_name, visible.name].filter(Boolean).forEach(name => names.add(name))
    })
    return [...names].sort()
  }, [plaidActuals, txOverrides, txRules, data.accounts])
  ...
  const cashForecastActualsByDate = useMemo(() => {
    const map = {}
    cashForecastActuals.forEach(transaction => {
      if (!map[transaction.date]) map[transaction.date] = []
      map[transaction.date].push(transaction)
    })
    return map
  }, [cashForecastActuals])

  // Historical ending balances are reconstructed from the verified live cash
  // anchor and exact-account posted rows. The helper owns Plaid sign semantics.
  const historicalBals = useMemo(() => {
    if (!forecastCanReconstructHistory || !cashForecastPostedActuals?.length) return {}
    const currentBal = forecastScope.accounts.reduce((sum, account) => {
      return sum + (Number.isFinite(account.plaidCurrentBalance) ? account.plaidCurrentBalance : parseFloat(account.balance || 0))
    }, 0)
    return reconstructHistoricalCashBalances({
      transactions:cashForecastPostedActuals,
  ...
  const adoptBalanceAttempt = useCallback(({ status, errors = [], checkedAt = '', live = false }) => {
    const snapshot = recordLatestBalanceRefreshStatus({
      balanceDataStatus:status,
      balanceErrors:errors,
      balanceCheckedAt:checkedAt,
      balanceMode:live ? LIVE_BALANCE_MODE : 'unknown',
      balanceProvenance:live ? LIVE_BALANCE_PROVENANCE : '',
    })
    setBalanceDataStatus(snapshot.status)
    setBalanceDataErrors(snapshot.errors)
    return snapshot
  }, [])
  ...
  const handlePlaidSync = useCallback(async (plaidAccounts, syncedAt, accountSourceReceipt, sourceErrors = []) => {
    ...
    const reportedAttempt = !plaidAccounts?.length
      ...
      : null
    if (reportedAttempt) {
      const status = String(reportedAttempt.balanceDataStatus).toLowerCase()
      const messages = sourceErrors.map(item => `${item.institution || 'Bank'}: ${item.message || 'balance refresh was not confirmed'}`)
      adoptBalanceAttempt({ status, checkedAt:syncedAt, live:false, errors:messages })
      return { ok:true, syncedAt, matchedCount:0, unmatchedCount:0, missingLinkedCount:0, partial:true }
    }
    // Build the source write from the exact persisted household record. The
    ...
    if (!next) {
      ...
      if (linkageIsOtherwiseSafe && missingLinkedCount) {
        const missingMessage = `${missingLinkedCount} previously linked Brevity ${missingLinkedCount === 1 ? 'account was' : 'accounts were'} missing from the live bank response. Prior balances and the last complete balance-check time were preserved.`
        adoptBalanceAttempt({ status:'partial', checkedAt:syncedAt, live:true, errors:[
          missingMessage,
    ...
    try {
      ...
      dataRef.current = current
      setData(current)
      adoptBalanceAttempt({ status:'stale', checkedAt:syncedAt, live:true, errors:[error.message || 'Account balances could not be synchronized safely.'] })
  ...
  const reviewBudgetChange = ({ line, year, month, value }) => stageDirectFinanceReview({
    summary: `Update ${line.name} budget for ${MONTHS[month]} ${year}`,
    operation: {
      type: 'budget.update',
      targetId: line.id,
      description: `Set ${line.name} budget for ${MONTHS[month]} ${year} to ${fmtMoney(value)}`,
      payload: {
        month,
        year,
        value,
        ...
        legacyYear:getHouseholdCalendarDate().getFullYear(),
  ...
  const reviewScheduledTransaction = async (tx, options = {}) => {
    ...
    const payload = buildScheduledActionPayload(tx, { current, occurrenceDate })
    if (current && !Object.keys(payload).length) {
      showToast('No scheduled changes to review')
      return false
    }
    if (current?.freq !== 'once' && options.requestedScope === 'one' && (Object.hasOwn(payload, 'frequency') || Object.hasOwn(payload, 'endDate'))) {
      showToast('Frequency and end-date changes require “This and future items.”')
      return false
    }
    const scope = scheduledActionScope(current, options)
    const isCreate = !current
    const recordLabel = tx?.type === 'transfer' ? 'forecast-only planned transfer' : 'cash-plan item'
    return stageDirectFinanceReview({
      summary: `${isCreate ? 'Create' : 'Update'} scheduled ${tx?.name || recordLabel}`,
      operation: {
        type:isCreate ? 'recurring.create' : 'recurring.update',
        targetId:current?.id || '',
        targetDate:occurrenceDate,
        description:`${isCreate ? 'Create' : 'Update'} the scheduled ${recordLabel} for ${tx?.name || 'this item'}`,
        payload,
        ...scope,
      },
      storageKey:LS_KEY,
      expectedVersion:Object.hasOwn(tx || {}, '_reviewVersion') ? tx._reviewVersion : undefined,
    })
  }

  const updateTx = async (tx) => {
    const reviewed = await reviewScheduledTransaction(tx)
  ...
  const deleteTx = async (transactionOrId, options = {}) => {
    const id = typeof transactionOrId === 'string' ? transactionOrId : transactionOrId?.id
    const current = dataRef.current.transactions.find(item => item.id === id)
    if (!current) {
      showToast('This scheduled record no longer exists. Refresh and try again.')
      return false
    }
    const occurrenceDate = options.occurrenceDate || current.start
    const scope = scheduledActionScope(current, options)
    return stageDirectFinanceReview({
      summary:`Delete scheduled ${current.name || 'cash-plan item'}`,
      operation:{
        type:'recurring.delete',
        targetId:current.id,
        targetDate:occurrenceDate,
        description:`Delete the scheduled cash-plan record for ${current.name || 'this item'}`,
        payload:{},
        ...scope,
      },
      storageKey:LS_KEY,
      expectedVersion:options.expectedVersion ?? (typeof transactionOrId === 'object' && Object.hasOwn(transactionOrId, '_reviewVersion') ? transactionOrId._reviewVersion : undefined),
    })
  }

  // Calendar changes stay on the calendar while Action Mode reviews the exact
  // occurrence, scope, and current shared finance-record version.
  const calendarSaveTx = (tx, options = {}) => reviewScheduledTransaction(tx, options)
  const calendarApplyScopedChange = ({ action, original, updated, occurrenceDate, scope }) => action === 'delete'
    ? deleteTx(original, { occurrenceDate, requestedScope:scope, occurrence:true })
    : reviewScheduledTransaction(updated, { occurrenceDate, requestedScope:scope, occurrence:true })
  const calendarMoveTx = ({ tx, fromDate, toDate, scope }) => reviewScheduledTransaction(
    { ...tx, start:toDate, end:tx.freq === 'once' ? toDate : tx.end },
    { occurrenceDate:fromDate, requestedScope:scope, occurrence:true },
  )
  ...
  const handleSaveActualTx = async (updated) => {
    if(readOnly)return false
    const payload={name:String(updated.name||'').trim(),category:String(updated.category||'').trim()}
    try{
      const result=await prepareDirectAction({
        summary:`Update ${payload.name||updated.originalStatement||'bank transaction'} metadata`,
        operation:{type:'transaction.update',targetId:updated.id,description:`Review the name and category for ${payload.name||'this transaction'}`,payload},
        expectedVersion:getAcknowledgedSharedStateVersion(localStorage,'lslj_tx_overrides_v1'),
      })
      requestActionReview(result.proposal)
      showToast('Review this transaction change in Action Mode')
      return true
    }catch(error){
      setStorageError(error.message||'This transaction change could not be prepared safely.')
      showToast('⚠ Transaction change not prepared')
      return false
    }
  }
  ...
  // Current-month recurring cash flow for the selected account scope. Count
  // each occurrence so partial-month starts/ends and five-paycheck months are
  // represented exactly.
  const monthlyTotals = useMemo(
    () => calculateMonthlyCashFlow(fd.transactions, todayKey),
    [fd.transactions, todayKey],
  )
  ...
  const operatingMonthlyExpense = useMemo(
    () => calculateMonthlyCashFlow(operatingTransactions, todayKey).recurringExpenses,
  ...
  const todayAlignment = useMemo(() => buildDailyAlignmentSnapshot({
    ...
    monthlyScheduled: fd.transactions,
    actuals: filteredActuals,
    budget: dailyBudget,
    budgetLegacyYear:t.getFullYear(),
    budgetLegacyAccountId:data.accounts.find(account => /\boperating\s+account\b/i.test(String(account?.name || '')))?.id || data.accounts.find(account => account.id === 'a1')?.id || '',
  }), [fd.accounts, fd.transactions, filteredActuals, dailyBudget, todayKey, data.accounts])
  ...
  const projectItems = hqItems.map(item => ({
    ...
    img:    item.dashboardImage || (ROOM_IMGS[item.room] || ROOM_IMG_DEFAULT),
  }))
  ...
  const nowDt = getHouseholdCalendarDate()
  ...
  const financialInsights = (() => {
    ...
    if (monthlyCashFlow > 1000) {
      ins.push({ type: 'cash-flow', sev: 'good', icon: 'ti-trending-up',
        title: `Recurring baseline cash flow: +${fmtMoney(monthlyCashFlow)}`,
        detail: `${fmtMoney(monthlyIncome)} recurring income · ${fmtMoney(monthlyExpense)} recurring expenses. One-time items are excluded; this is a forecast, not posted cash.`,
      ...
      ins.push({ type: 'cash-flow', sev: 'danger', icon: 'ti-trending-down',
        title: `Recurring baseline shortfall: −${fmtMoney(Math.abs(monthlyCashFlow))}`,
        detail: `${fmtMoney(monthlyIncome)} recurring income vs ${fmtMoney(monthlyExpense)} recurring expenses. One-time items are excluded; this is a forecast, not posted cash.`,
        action: 'Spending exceeds income. Review recurring expenses for cuts.',
        monthlyForecasts, accounts: fd.accounts })
    } else {
      ins.push({ type: 'cash-flow', sev: 'tip', icon: 'ti-chart-bar',
        title: `Recurring baseline: ${fmtMoney(monthlyIncome)}/mo in · ${fmtMoney(monthlyExpense)}/mo out`,
        detail: `Recurring monthly net: ${monthlyCashFlow >= 0 ? '+' : '−'}${fmtMoney(Math.abs(monthlyCashFlow))}. One-time items are excluded. Click to see the year-end balance forecast.`,
  ...
  const todayLabel = getHouseholdDateLabel()

  const isForm = view === 'tx-form'
  ...
  const AccountFilterBar = !isForm && (
    <div className="finance-account-filter" style={{
      ...
      {view !== 'calendar' && (
        <>
          {/* Cash Forecast owns its month-scoped bank-activity toggle. */}
          ...
          <button
            ...
            aria-label={`${showActuals ? 'Hide' : 'Show'} bank activity`}
            style={{
            ...
            {actualsLoading ? 'Loading…' : 'Bank activity'}
          </button>
        </>
      )}
    ...
    <div className={`finance-root fade-in${view === 'dashboard' ? '' : ' finance-scroll'}`}>
      ...
      {!formView && view === 'transactions' && unmappedActuals.length > 0 && (
        <div role="status" style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10, margin:'10px 28px 0', padding:'10px 13px', borderRadius:10, border:'1px solid rgba(232,150,122,.30)', background:'rgba(232,150,122,.08)', color:'var(--soft-white)', fontSize:11, lineHeight:1.45 }}>
          ...
          {!allSelected && <button type="button" onClick={openAllBankActivity} style={{ border:'1px solid rgba(197,164,109,.35)', background:'rgba(197,164,109,.10)', color:'var(--gold)', borderRadius:9, padding:'7px 10px', cursor:'pointer', fontFamily:'inherit' }}>Open all bank activity</button>}
        </div>
      )}
      ...
      {view === 'dashboard' && (
        <div className="dash-body">
          ...
          <div className="dash-header">
            <div>
              <div className="dash-greeting">{getHouseholdGreeting()}, {currentMember}</div>
            ...
            <div className="dash-actions">
              ...
              <button type="button" className="dash-icon-btn" title="Upcoming financial calendar" aria-label="Open upcoming financial calendar" onClick={() => setView('calendar')}><i className="ti ti-bell" aria-hidden="true" /></button>
              {!readOnly && <button type="button" className="dash-icon-btn gold" title="Add transaction" aria-label="Add transaction" onClick={() => { setEditTx(null); setView('tx-form') }}>
          ...
          <div className="dash-main-grid">
            ...
            <div className="dash-card" style={{ gridColumn: '1 / 3', gridRow: '1 / 3', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <button aria-label="Previous dashboard calendar month" onClick={mcNavPrev} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 24, padding: '0 6px', lineHeight: 1 }}>‹</button>
                <div style={{ fontSize: 22, fontWeight: 600, color: 'var(--white)', letterSpacing: '0.01em' }}>{mcMonthName}</div>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <button aria-label="Next dashboard calendar month" onClick={mcNavNext} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 24, padding: '0 6px', lineHeight: 1 }}>›</button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4, flex: 1 }}>
                {mcCells.map((d, i) => {
                  ...
                  const hasTransfers = showActuals && dayActuals.some(isTransferTransaction)
                  const dayCashActivity = showActuals ? summarizeActualCashActivity(dayActuals) : null
                  // Bank credits are cash inflows, but only recognized posted
                  // income appears as realized income elsewhere on the dashboard.
                  const dayInc = showActuals
                    ? dayCashActivity.inflows
                    : (pt?.txns?.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0) || 0)
                  const dayExp = showActuals
                    ? dayCashActivity.expenses
                    ...
                    <div key={key} onClick={() => { setView('calendar'); setSelDay(key) }}
                      ...
                      {hasTx && (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                          ...
                          {showActuals && hasTransfers && <div style={{ fontSize: 9, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'.06em' }}>transfer</div>}
              ...
              <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--glass-border)', display: 'flex', gap: 20, justifyContent: 'center' }}>
                {[['rgba(197,164,109,0.90)',showActuals ? 'Cash in' : 'Income'],['rgba(247,243,234,0.70)','Expense'], ...(showActuals ? [['var(--muted)','Transfers excluded']] : [])].map(([c,l]) => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, overflowY: 'auto', flex: 1 }}>
                {showActuals ? (
                  ...
                  timeframeActuals.length === 0
                    ? <p style={{ fontSize: 14, color: 'var(--muted)' }}>{plaidActuals ? 'No recent transactions found.' : 'No connected bank source is available. Connection changes are disabled in this release.'}</p>
                    : sortAndFilterTransactions(timeframeActuals).slice(0, 14).map((tx, i) => {
                        const activityKind = actualTransactionKind(tx)
                        const cashIn = Number(tx.amount) < 0
                        const activityLabel = tx.pending
                          ? (isRecognizedIncomeTransaction(tx) ? 'Pending income · not realized' : cashIn ? 'Pending refund / other credit' : 'Pending expense')
                          : activityKind === 'income' ? 'Realized income' : activityKind === 'other-inflow' ? 'Refund / other credit' : activityKind === 'transfer' ? 'Transfer' : 'Posted expense'
                        const activityColor = activityKind === 'income' ? 'var(--income-color)' : activityKind === 'expense' ? 'var(--expense-color)' : 'var(--muted)'
                        return (
                          <div key={i}
                            onClick={() => !readOnly && openActualTxModal(tx)}
                            style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: readOnly ? 'default' : 'pointer', borderRadius: 8, padding: '4px 6px', margin: '-4px -6px', transition: 'background .15s' }}
                            onMouseEnter={e => { if (!readOnly) e.currentTarget.style.background = 'rgba(255,255,255,0.06)' }}
                            onMouseLeave={e => { e.currentTarget.style.background = '' }}>
                            <div style={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, background: activityKind === 'income' ? 'rgba(197,164,109,0.14)' : activityKind === 'expense' ? 'rgba(196,120,90,0.12)' : 'rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <p style={{ margin: 0, fontSize: 11, color: 'var(--muted)' }}>
                                ...
                                <span style={{ marginLeft:6 }}>{activityLabel}</span>
                                {tx.pending && <span style={{ marginLeft: 6, color: 'rgba(197,164,109,0.75)', fontSize: 10 }}>Pending</span>}
                              </p>
                            </div>
                            <span style={{ fontSize: 13, fontWeight: 600, color: activityColor, flexShrink: 0 }}>
                              {cashIn ? '+' : '-'}{fmtMoney(Math.abs(tx.amount))}
                            </span>
                          </div>
                        )
                      })
            ...
            <div className="dash-card" style={{ gridColumn: '3', gridRow: '2' }}>
              ...
              {!readOnly && <button type="button" disabled title="Project image changes are unavailable until reviewed uploads can be audited and safely undone." style={{ marginTop: 10, width: '100%', padding: '10px 12px', background: 'transparent', border: '1px solid var(--glass-border)', borderRadius: 10, color: 'var(--muted)', fontSize: 12, fontWeight: 500, cursor: 'not-allowed', fontFamily: 'var(--font)' }}>
                Project images are view-only until reviewed image changes support Audit History and safe Undo.
              </button>}
            </div>
            ...
            <div className="dash-card" style={{ gridColumn: '1 / 4', gridRow: '4' }}>
              ...
              {financialInsights.length === 0 ? (
                <p style={{ fontSize: 13, color: 'var(--muted)', fontStyle: 'italic' }}>
                  Add projected transactions to build planning insights. No connected bank source is available, and connection changes are disabled in this release.
                </p>
              ) : (
          ...
          {insightModal && (
            <div className="finance-insight-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1200,
              display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
              onClick={() => setInsightModal(null)}>
              <div className="finance-insight-dialog" onClick={e => e.stopPropagation()} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                  ...
                  <button aria-label="Close financial insight" onClick={() => setInsightModal(null)} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--muted)', cursor: 'pointer', fontSize: 14, padding: '4px 10px', borderRadius: 8, lineHeight: 1.2 }}>&#x2715;</button>
                </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {sortAndFilterTransactions(insightModal.transactions).map((tx, idx) => {
                        ...
                        return (
                          <div key={idx}
                            onClick={() => !readOnly && !isPlaid && setInsightEditTx({ ...tx, _reviewVersion:captureFinanceReviewVersion() })}
                            style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                              padding: '10px 14px', background: 'rgba(255,255,255,0.04)', borderRadius: 10,
                              border: '1px solid rgba(255,255,255,0.07)',
                              cursor: readOnly || isPlaid ? 'default' : 'pointer', transition: 'background .12s' }}
                            onMouseEnter={e => { if (!readOnly && !isPlaid) e.currentTarget.style.background = 'rgba(255,255,255,0.08)' }}
                            ...
                            <div style={{ minWidth: 0, flex: 1 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                ...
                                {!readOnly && !isPlaid && <i className="ti ti-pencil" style={{ fontSize: 11, color: 'rgba(197,164,109,0.5)', flexShrink: 0 }} />}
                              </div>
          ...
          {!readOnly && insightEditTx && (
            <div className="finance-insight-overlay finance-insight-edit-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.78)', zIndex: 1300,
              display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
              ...
              <div className="finance-insight-dialog" onClick={e => e.stopPropagation()} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  ...
                  <button aria-label="Close transaction editor" onClick={() => setInsightEditTx(null)} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--muted)', cursor: 'pointer', fontSize: 14, padding: '4px 10px', borderRadius: 8 }}>&#x2715;</button>
                </div>
                <TxForm tx={insightEditTx} accounts={data.accounts}
                  onSave={async tx => { if (await calendarSaveTx(tx)) setInsightEditTx(null) }}
                  onCancel={() => setInsightEditTx(null)} />
                <button
                  onClick={async () => { if (await deleteTx(insightEditTx)) { setInsightEditTx(null); setInsightModal(null) } }}
      ...
      {view === 'daily-alignment' && (
        <DailyAlignment
          readOnly={readOnly}
          financeReadOnly={readOnly}
          meetingPlanningReadOnly={meetingPlanningReadOnly}
          currentMember={currentMember}
          accounts={fd.accounts}
          scheduled={fd.transactions}
          cashFlowScheduled={fd.transactions}
          actuals={filteredActuals}
          balanceDataStatus={balanceDataStatus}
          transactionFreshnessStatus={actualsFreshness?.status || 'unknown'}
          budget={dailyBudget}
          budgetLegacyYear={t.getFullYear()}
          budgetLegacyAccountId={data.accounts.find(account => /\boperating\s+account\b/i.test(String(account?.name || '')))?.id || data.accounts.find(account => account.id === 'a1')?.id || ''}
          projection={proj}
      ...
      {view === 'scenario-modeling' && (
        <ScenarioModeling liveOperatingExpense={operatingMonthlyExpense} readOnly={readOnly} />
      )}
      ...
      {view === 'calendar' && (
        <div className="finance-inner">
          <CalendarView proj={cashForecastProjection} calYear={calYear} calMonth={calMonth}
            ...
            accounts={data.accounts} viewAcctIds={forecastScope.accountIds} onSave={calendarSaveTx} onApplyScopedChange={calendarApplyScopedChange} onMove={calendarMoveTx} onDelete={deleteTx}
            showActuals={showActuals} toggleActuals={toggleActuals} actualsLoading={actualsLoading} actualsError={actualsError}
            actualsFreshnessMessage={actualsFreshnessMessage} actualsFreshnessStatus={actualsFreshness?.status}
            balanceDataMessage={balanceDataMessage} balanceDataStatus={balanceDataStatus}
            actualsByDate={cashForecastActualsByDate} plaidActuals={cashForecastActuals}
            historicalBals={historicalBals}
            excludedAccountCount={fd.accounts.length - forecastScope.accounts.length}
            unmappedTransactionCount={unmappedActuals.length}
            balanceSource={forecastBalanceSource}
        ...
        <div className="finance-inner">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap:'wrap', gap:10, marginBottom: 12 }}>
            <p style={{ fontSize: 14, fontWeight: 600 }}>
              {showActuals
                ? transactionFilter?.realizedIncomeOnly
                  ? `${transactionViewActuals.length} posted income transaction${transactionViewActuals.length !== 1 ? 's' : ''}`
          ...
          {showActuals&&(actualsNotice||actualsError||actualsFreshnessMessage)&&<div role={actualsError||actualsFreshness?.status==='stale'?'alert':'status'} style={{padding:'10px 13px',marginBottom:14,borderRadius:10,border:`1px solid ${actualsError||actualsFreshness?.status==='stale'?'rgba(232,150,122,.35)':'rgba(197,164,109,.2)'}`,background:actualsError||actualsFreshness?.status==='stale'?'rgba(232,150,122,.08)':'rgba(197,164,109,.06)',color:actualsError||actualsFreshness?.status==='stale'?'var(--expense-color)':'var(--muted)',fontSize:11,lineHeight:1.45}}>{actualsError||actualsFreshnessMessage||actualsNotice}</div>}
          ...
          {transactionFilter && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '12px 16px', marginBottom: 16, borderRadius: 12, background: 'rgba(197,164,109,.08)', border: '1px solid rgba(197,164,109,.2)' }}>
              <div><strong style={{ color: 'var(--gold)' }}>{transactionFilter.label || 'Filtered transactions'}</strong><div style={{ color: 'var(--muted)', fontSize: 11, marginTop: 3 }}>{transactionFilter.budgetLines ? `${scheduledViewTransactions.length} of ${transactionFilter.budgetLines.length} budget lines shown · ${fmtMoney(scheduledViewStats.income + scheduledViewStats.expenses)} shown of ${fmtMoney(transactionFilter.budgetTotal)} card total` : `${showActuals?transactionViewActuals.length:scheduledViewTransactions.length} matching transactions in the selected timeframe`}</div></div>
          ...
          <div className="finance-summary-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 18 }}>
            <div role="button" tabIndex={0} onClick={() => showActuals
              ? openFilteredTransactions({ ...(transactionFilter || {}), direction:'income', label:transactionFilter?.realizedIncomeOnly || transactionFilter?.profitAndLossOnly ? 'Realized income' : transactionFilter?.postedOnly ? 'Posted cash inflows' : 'Cash inflows' })
              : openScheduledTransactions({ direction:'income', recurringOnly:Boolean(transactionFilter?.recurringOnly), range:transactionFilter?.range || financeRange, label:transactionFilter?.recurringOnly ? 'Recurring income' : 'Scheduled income' })} style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.04)', borderRadius: 14, border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer' }}>
              <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 600, color: 'var(--income-color)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{showActuals ? transactionFilter?.realizedIncomeOnly || transactionFilter?.profitAndLossOnly ? 'Realized Income' : transactionFilter?.postedOnly ? 'Posted Cash Inflows' : 'Cash Inflows · Incl. Pending' : 'Expected Income'}</p>
              <p style={{ margin: 0, fontSize: 24, fontWeight: 300, color: 'var(--white)', letterSpacing: '-0.02em' }}>{fmtMoney(showActuals ? transactionViewStats.income : scheduledViewStats.income)}</p>
            </div>
            <div role="button" tabIndex={0} onClick={() => showActuals
              ? openFilteredTransactions({ ...(transactionFilter || {}), direction:'expense', label:transactionFilter?.postedOnly || transactionFilter?.profitAndLossOnly ? 'Posted expenses' : 'Expenses including pending' })
              : openScheduledTransactions({ direction:'expense', recurringOnly:Boolean(transactionFilter?.recurringOnly), range:transactionFilter?.range || financeRange, label:transactionFilter?.recurringOnly ? 'Recurring expenses' : 'Scheduled expenses' })} style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.04)', borderRadius: 14, border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer' }}>
              <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 600, color: 'var(--expense-color)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{showActuals ? transactionFilter?.postedOnly || transactionFilter?.profitAndLossOnly ? 'Posted Expenses' : 'Expenses · Incl. Pending' : transactionFilter?.recurringOnly ? 'Recurring Expenses' : 'Expected Expenses'}</p>
              <p style={{ margin: 0, fontSize: 24, fontWeight: 300, color: 'var(--white)', letterSpacing: '-0.02em' }}>{fmtMoney(showActuals ? transactionViewStats.expenses : scheduledViewStats.expenses)}</p>
            </div>
            <div role="button" tabIndex={0} onClick={() => showActuals
              ? openFilteredTransactions(transactionFilter ? { ...transactionFilter, direction:null, label:transactionFilter.realizedIncomeOnly || transactionFilter.profitAndLossOnly ? 'Posted profit and loss' : transactionFilter.postedOnly ? 'Posted cash flow' : 'Cash flow including pending' } : null)
              : openScheduledTransactions(transactionFilter?.recurringOnly ? { recurringOnly:true, range:transactionFilter?.range || financeRange, label:'Recurring cash flow' } : null)} style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.04)', borderRadius: 14, border: `1px solid ${transactionSummaryNet >= 0 ? 'rgba(197,164,109,0.20)' : 'rgba(196,120,90,0.20)'}`, cursor: 'pointer' }}>
              <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 600, color: transactionSummaryNet >= 0 ? 'var(--income-color)' : 'var(--expense-color)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Net Cash Flow</p>
              <p style={{ margin: 0, fontSize: 24, fontWeight: 300, color: transactionSummaryNet >= 0 ? 'var(--gold-light)' : 'var(--expense-color)', letterSpacing: '-0.02em' }}>{fmtMoney(transactionSummaryNet)}</p>
            </div>
          </div>

          {showActuals && transactionFilter && transactionHistory.length > 0 && (
            <div className="finance-card" style={{ padding: 16, marginBottom: 18 }}>
              <p style={{ margin: '0 0 10px', fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.1em' }}>History by month</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 8 }}>{transactionHistory.map(([month, totals]) => <div key={month} style={{ padding: 10, borderRadius: 9, background: 'rgba(255,255,255,.035)' }}><strong>{month}</strong><div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>{totals.count} transactions · {fmtMoney(totals.total)}</div></div>)}</div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {showActuals ? (
              ...
              transactionViewActuals.length === 0
                ? <p style={{ fontSize: 14, color: 'var(--muted)', padding: '20px 0' }}>{plaidActuals ? 'No transactions found for selected accounts.' : 'No connected bank source is available for posted transactions. Connection changes are disabled in this release.'}</p>
                : transactionViewActuals.map((tx, i) => {
                    const activityKind = actualTransactionKind(tx)
                    const cashIn = Number(tx.amount) < 0
                    const activityLabel = tx.pending
                      ? (isRecognizedIncomeTransaction(tx) ? 'Pending income · not realized' : cashIn ? 'Pending refund / other credit' : 'Pending expense')
                    ...
                    return (
                      <div key={tx.id || i}
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px',
                          ...
                          borderRadius: 10, cursor: readOnly ? 'default' : 'pointer', transition: 'background .15s' }}
                        onMouseEnter={e => { if (!readOnly) e.currentTarget.style.background = 'rgba(255,255,255,0.08)' }}
                        onMouseLeave={e => { e.currentTarget.style.background = tx.pending ? 'rgba(197,164,109,0.05)' : 'rgba(255,255,255,0.04)' }}>
                        <div style={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, background: activityKind === 'income' ? 'rgba(197,164,109,0.12)' : activityKind === 'expense' ? 'rgba(196,120,90,0.12)' : 'rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <i className={`ti ti-${activityKind === 'transfer' ? 'arrows-exchange' : cashIn ? 'arrow-down-left' : 'arrow-up-right'}`} style={{ fontSize: 14, color: activityColor }} />
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 600, flexShrink: 0, color: activityColor }}>
                          {cashIn ? '+' : '-'}{fmtMoney(Math.abs(tx.amount))}
                        </p>
                        {!readOnly && <i className="ti ti-chevron-right" style={{ fontSize: 14, color: 'var(--muted)', flexShrink: 0 }} />}
                      </div>
                    )
                  })
            ) : (
              ...
              scheduledViewTransactions.map(tx => {
                  ...
                  return (
                    <div key={tx.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, transition: 'background 0.15s' }}>
                      ...
                      {!readOnly && !tx.budgetLine && <div style={{ display: 'flex', gap: 4 }}>
                        <button aria-label={`Edit ${tx.name}`} onClick={() => { setEditTx({ ...tx }); setView('tx-form') }}
                        ...
                        <button aria-label={`Delete ${tx.name}`} onClick={() => deleteTx(tx.id)}
                          style={{ padding: '5px 8px', cursor: 'pointer', borderRadius: 7, border: '1px solid rgba(255,255,255,0.09)', background: 'transparent', color: 'var(--muted)' }}
      ...
      {view === 'accounts' && (
        <div className="finance-inner">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 500 }}>
                    ...
                    <span style={{ marginLeft: 8, fontSize: 10, fontWeight: 600, background: acct.plaidAccountId ? 'rgba(125,203,164,.12)' : 'rgba(255,255,255,.06)', color: acct.plaidAccountId ? '#7DCBA4' : 'var(--muted)', padding: '2px 7px', borderRadius: 10 }}>
                      {acct.plaidAccountId ? 'Bank sourced' : 'Not linked'}
                    </span>
                  </p>
                  <p style={{ margin: 0, fontSize: 11, color: 'var(--muted)' }}>
          ...
          <div className="finance-card">
            ...
            <div className="finance-accounts-forecast" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
              {[
      ...
      {view === 'budget' && (
        <div className="finance-inner">
          <BudgetView
            ...
            onOpenRecurring={openScheduledTransactions}
            onReviewBudgetChange={reviewBudgetChange}
            readOnly={readOnly}
          />
        </div>
      )}
...
function BudgetView({ data, plaidActuals = [], legacyAccountId = '', initialMonth, onOpenTransactions, onOpenRecurring, onReviewBudgetChange, readOnly = false }) {
  ...
  useEffect(() => {
    const refreshBudget = event => {
      const key = event?.detail?.key || event?.key
      if (!key || key === BUDGET_LS_KEY) setBudget(loadBudget())
    }
    window.addEventListener(SHARED_STATE_EVENT, refreshBudget)
    window.addEventListener('storage', refreshBudget)
    return () => {
      window.removeEventListener(SHARED_STATE_EVENT, refreshBudget)
      window.removeEventListener('storage', refreshBudget)
    }
  }, [])
  ...
  const lineActual = line => actualAllocation.byLine[line.id] || 0
  const catActual = cat => syncedActualsByCategory[cat] || 0

  const totalBudget  = Object.keys(BUDGET_CATS).filter(c => c !== 'Income').reduce((s, c) => s + catBudgeted(c), 0)
  const totalActual  = Object.entries(syncedActualsByCategory).filter(([category]) => category !== 'Income').reduce((sum, [, amount]) => sum + amount, 0)
  ...
  const openBudgetBreakdown = (direction) => {
    ...
    onOpenRecurring?.({
      ...
      budgetTotal: budgetBreakdownTotal(breakdownLines),
  ...
  if (mode === 'monthly-view') {
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '4px 12px' }}>
              <button aria-label="Previous budget month" onClick={() => { const d = new Date(selYear, selMonth - 1); setSelMonth(d.getMonth()); setSelYear(d.getFullYear()) }}
              ...
              <button aria-label="Next budget month" onClick={() => { const d = new Date(selYear, selMonth + 1); setSelMonth(d.getMonth()); setSelYear(d.getFullYear()) }}
                style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 16, padding: '0 4px' }}>›</button>
            </div>
        ...
        {/* Summary cards */}
        <div className="finance-budget-summary" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 28 }}>
          {[
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {expenseCats.map(([cat, lines]) => {
            const budgeted = catBudgeted(cat)
            const actual   = catActual(cat)
            const pct      = budgeted > 0 ? Math.min((actual / budgeted) * 100, 100) : actual > 0 ? 100 : 0
            const over     = actual > budgeted
            const unallocated = actualAllocation.unallocatedByCategory[cat] || 0
            ...
            return (
              <div key={cat} style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${over ? 'rgba(248,113,113,0.25)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 14, overflow: 'hidden' }}>
                {/* Category header */}
                <div className="finance-budget-category-header" onClick={() => setExpanded(p => ({ ...p, [cat]: !isOpen }))}
                  ...
                  <div className="finance-budget-category-metrics" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: over ? 'var(--expense-color)' : 'var(--muted)', minWidth: 60, textAlign: 'right' }}>{budgeted > 0 ? `${Math.round(pct)}%` : actual > 0 ? 'Unbudgeted' : '—'}</span>
                ...
                {isOpen && (
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    {lines.map(line => {
                      const b = getBudgeted(line)
                      const a = lineActual(line)
                      ...
                      return (
                        <div className="finance-budget-line" key={line.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 20px 10px 40px', borderTop: '1px solid rgba(255,255,255,0.03)' }}>
                          ...
                          <span className="finance-budget-line-percent" style={{ fontSize: 11, color: 'var(--muted)', minWidth: 32, textAlign: 'right' }}>{Math.round(p)}%</span>
                          <div
                            className="finance-budget-line-actual"
                            data-label="Actual"
                            title="Calculated from posted bank activity; pending transactions are excluded"
                            style={{ width: 80, textAlign: 'right', fontSize: 12, padding: '3px 8px', borderRadius: 6,
                              color: a ? (ov ? 'var(--expense-color)' : 'var(--white)') : 'rgba(255,255,255,0.2)',
                              border: '1px solid transparent' }}>
                            {a ? fmtMoney(a) : '—'}
                          </div>
                          <span className="finance-budget-line-budget" data-label="Budget" style={{ fontSize: 12, color: 'var(--muted)', minWidth: 80, textAlign: 'right' }}>{b ? fmtMoney(b) : '—'}</span>
                        </div>
                      )
                    })}
                    {unallocated > 0 && (
                      <div className="finance-budget-line" style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 20px 10px 40px', borderTop:'1px solid rgba(255,255,255,0.03)' }}>
                        ...
                        <span className="finance-budget-line-budget" data-label="Budget" style={{ minWidth:80, textAlign:'right', fontSize:12, color:'var(--muted)' }}>—</span>
                      </div>
                    )}
                    <div className="finance-budget-category-total" style={{ display: 'flex', padding: '8px 20px 10px 40px', borderTop: '1px solid rgba(255,255,255,0.05)', gap: 12 }}>
                      <span className="finance-budget-category-total-label" style={{ flex: 1, fontSize: 11, color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Category Total</span>
                      <span className="finance-budget-category-total-actual" style={{ fontSize: 12, fontWeight: 700, color: over ? 'var(--expense-color)' : 'var(--income-color)', minWidth: 80, textAlign: 'right' }}>{fmtMoney(actual)}</span>
                      <span className="finance-budget-category-total-budget" style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted)', minWidth: 80, textAlign: 'right' }}>{fmtMoney(budgeted)}</span>
                    </div>
                  </div>
                )}
              </div>
  ...
  const getCellVal = (line, month) => getBudgeted(line, month, selYear)

  const reviewCellVal = async (line, month, val) => {
    if (readOnly) return false
    const num = parseFloat(val) || 0
    await onReviewBudgetChange?.({ line, year:selYear, month, value:num })
    return true
  }

  const annualTotal = (line) => {
    return MONTHS.reduce((sum, _, month) => sum + getBudgeted(line, month, selYear), 0)
  }

  const catTotal = (cat, month) => {
    return (BUDGET_CATS[cat] || []).reduce((s, line) => {
      const v = month === 'annual'
        ? annualTotal(line)
        : parseFloat(getCellVal(line, month) || 0)
      return s + v
    }, 0)
  }
    ...
    <div>
      ...
      <div style={{ overflowX: 'auto', borderRadius: 16, border: '1px solid rgba(255,255,255,0.07)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: period === 'monthly' ? 1100 : 500 }}>
          ...
          <tbody>
            {Object.entries(BUDGET_CATS).map(([cat, lines]) => {
                      ...
                      {period === 'monthly'
                        ? MONTHS.map((_, mi) => (
                          <td key={mi} style={{ padding: '4px' }}>
                            {!readOnly && editCell?.lineId === line.id && editCell?.month === mi ? (
                              <input type="number" autoFocus
                                defaultValue={getCellVal(line, mi)}
                                onBlur={e => { reviewCellVal(line, mi, e.target.value); setEditCell(null) }}
                                onKeyDown={e => {
                              ...
                              <div onClick={() => !readOnly && setEditCell({ lineId:line.id, month:mi })}
                                style={{ cursor: readOnly ? 'default' : 'text', textAlign: 'right', fontSize: 12, padding: '3px 6px', borderRadius: 6,
                                  color: getCellVal(line, mi) !== '' ? 'var(--white)' : 'rgba(255,255,255,0.15)',
                                  border: '1px solid transparent' }}
                                onMouseEnter={e => { if (!readOnly) e.currentTarget.style.borderColor = 'rgba(255,255,255,0.12)' }}
                                onMouseLeave={e => e.currentTarget.style.borderColor = 'transparent'}>
                                {getCellVal(line, mi) !== '' ? fmtMoney(getCellVal(line, mi)) : '—'}
                              </div>
                            )}
                          </td>
                        ))
                        : (
                          <td style={{ padding: '4px' }}>
                            <div
                              title="Calculated from reviewed monthly targets"
                              style={{ cursor: 'default', textAlign: 'right', fontSize: 12, padding: '3px 6px', borderRadius: 6,
                                color: annualTotal(line) ? 'var(--white)' : 'rgba(255,255,255,0.15)',
                                border: '1px solid transparent' }}>
                              {annualTotal(line) ? fmtMoney(annualTotal(line)) : '—'}
                            </div>
                          </td>
                        )}
                      <td style={{ padding: '7px 4px', fontSize: 12, fontWeight: 600, textAlign: 'right', color: annualTotal(line) ? 'var(--soft-white)' : 'rgba(255,255,255,0.15)' }}>
                        {annualTotal(line) ? fmtMoney(annualTotal(line)) : '—'}
                      </td>
                    </tr>
                  ))}
      ...
      <p style={{ marginTop: 10, fontSize: 11, color: 'var(--muted)' }}>{readOnly ? 'Budget targets are read-only for household members.' : 'Editing a monthly target opens Action Mode review · unreviewed months follow the recurring plan · annual totals are calculated from all monthly budget values'}</p>
    </div>
  )
}
...
function CashFlowReport({ plaidActuals = [], onOpenTransactions }) {
  ...
  const [year,     setYear]     = useState(() => getHouseholdCalendarDate().getFullYear())
  ...
  const yearActuals = useMemo(() => {
    return plaidActuals.filter(t => {
      if (t.pending || isTransferTransaction(t)) return false
  ...
  const open = useCallback((filter = {}) => onOpenTransactions?.({
    ...
    label:filter.direction === 'income' ? 'Cash inflows' : filter.direction === 'expense' ? 'Expenses' : 'Net cash flow',
    ...filter,
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--muted)' }}>Posted bank inflows and outflows from connected accounts; pending activity and transfers are excluded</p>
        </div>
        {/* Period toggle */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20, marginBottom: 24 }}>
        <button aria-label="Previous report year" onClick={() => { setYear(y => y - 1); setDrillCat(null) }}
        ...
        <button aria-label="Next report year" onClick={() => { setYear(y => y + 1); setDrillCat(null) }}
          style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 8, color: 'var(--muted)', cursor: 'pointer', fontSize: 16, padding: '6px 12px', display: 'flex', alignItems: 'center' }}>
        ...
        {noData ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 20, justifyContent: 'center', marginTop: 6 }}>
              {[['#4CAF50', false, 'Cash Inflows'], ['#F44060', false, 'Expenses'], ['var(--gold)', true, 'Net Cash Flow']].map(([c, isLine, l]) => (
      ...
      {/* Summary stats */}
      <div className="finance-cashflow-summary" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 28 }}>
        {[
          { label: 'Cash Inflows',  val: fmt(totalIncome),                color: '#4CAF50', filter: { direction: 'income' } },
      ...
      <div className="finance-cashflow-breakdowns" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: drillCat ? 16 : 0 }}>
        {/* Cash inflow breakdown */}
        <div className="finance-card" style={{ padding: '20px 22px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: 'var(--white)' }}>Cash Inflows</h3>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#4CAF50' }}>{fmt(totalIncome)}</span>
          </div>
          {incomeByCategory.length === 0 ? (
            <p style={{ fontSize: 12, color: 'var(--muted)', margin: 0 }}>No cash inflows for {year}</p>
        ...
        <div className="finance-card" style={{ padding: '20px 22px', marginTop: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <button aria-label="Close category details" onClick={() => setDrillCat(null)}
...
function ReportingView({ data, proj, plaidActuals = [], legacyAccountId = '', onOpenTransactions, onOpenAccounts }) {
  ...
  const reportActuals = useMemo(
    () => currentYearActuals.filter(transaction => budgetCategoryForTransaction(transaction)),
    [currentYearActuals],
  )
  const actualMonthCount = mo + 1
  const balanceSheet = useMemo(() => buildBalanceSheet(data.accounts), [data.accounts])
  const hasActuals = reportActuals.length > 0
  ...
  const budgetMonthly = useMemo(() => {
    const map = {}
    const comparisonMonths = hasActuals ? Array.from({ length:mo + 1 }, (_, month) => month) : [mo]
    Object.entries(BUDGET_CATS).forEach(([cat, lines]) => {
      map[cat] = lines.reduce((sum, line) => {
        const targets = budgetTargetRow({ budget, line, year:yr, legacyYear:yr, legacyAccountId, legacyOwners })
        const comparableTotal = comparisonMonths.reduce((total, month) => {
          const value = targets[month]
          return total + (value !== undefined
            ? Math.max(Number(value) || 0, 0)
            : line.transactions.reduce((lineTotal, transaction) => lineTotal + calculateTransactionAmountForMonth(transaction, new Date(yr, month, 1), { recurringOnly:true }), 0))
        }, 0)
        return sum + comparableTotal / comparisonMonths.length
      }, 0)
    })
    return map
  }, [BUDGET_CATS, budget, hasActuals, legacyAccountId, legacyOwners, mo, yr])
  const reportBudgetCategories = useMemo(
    () => [...new Set([...Object.keys(BUDGET_CATS), ...Object.keys(normalizedActuals)])],
    [BUDGET_CATS, normalizedActuals],
  )
  ...
  return (
    <div>
      {/* Tab nav */}
      <div className="finance-report-tabs" role="tablist" aria-label="Business statement views" style={{ display: 'flex', gap: 0, borderBottom: '1px solid rgba(255,255,255,0.07)', marginBottom: 28 }}>
        {REPORT_TABS.map(t => (
          <button type="button" role="tab" aria-selected={tab === t.id} key={t.id} onClick={() => setTab(t.id)}
      ...
      {tab === 'overview' && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: 12, marginBottom: 20 }}>
            {[
              ['Revenue', hasActuals ? actualReport.income : plannedAnnual.income, 'var(--gold)', false, { direction:'income', realizedIncomeOnly:true, label:'Realized revenue' }],
              ['Expenses', hasActuals ? actualReport.expenses : plannedAnnual.expenses, 'var(--expense-color)', false, { direction:'expense', postedOnly:true, label:'Posted expenses' }],
              ['Net Profit', hasActuals ? actualReport.net : plannedAnnual.net, (hasActuals ? actualReport.net : plannedAnnual.net) >= 0 ? 'var(--gold)' : 'var(--expense-color)', false, { profitAndLossOnly:true, excludeTransfers:true, label:'Net Profit' }],
              ...
              <tbody>
                {[
                  ...
                  <tr key={row.label} style={{ borderTop: '1px solid rgba(255,255,255,0.05)', background: row.isNet ? 'rgba(197,164,109,0.04)' : 'transparent' }}>
                    ...
                    {row.vals.map((v, i) => (
                      <td key={i} role="button" tabIndex={0} title="Open matching transactions" onClick={() => onOpenTransactions?.({ direction: row.direction, postedOnly:true, ...(row.direction === 'income' ? { realizedIncomeOnly:true } : row.isNet ? { profitAndLossOnly:true } : {}), excludeTransfers:true, dateFrom:`${yr}-${String(i + 1).padStart(2, '0')}-01`, dateTo:`${yr}-${String(i + 1).padStart(2, '0')}-${String(new Date(yr, i + 1, 0).getDate()).padStart(2, '0')}`, label:`${row.label} · ${MONTHS[i]} ${yr}` })} style={{ padding: '10px 6px', fontSize: 11, fontWeight: 600, textAlign: 'right', cursor: 'pointer',
                        color: row.isNet ? (v >= 0 ? 'var(--gold)' : 'var(--expense-color)') : row.color }}>
                        {fmtMoney(v)}
                      </td>
                    ))}
                    <td role="button" tabIndex={0} title="Open matching transactions" onClick={() => onOpenTransactions?.({ direction:row.direction, postedOnly:true, ...(row.direction === 'income' ? { realizedIncomeOnly:true } : row.isNet ? { profitAndLossOnly:true } : {}), excludeTransfers:true, dateFrom:`${yr}-01-01`, dateTo:`${yr}-12-31`, label:`${row.label} · ${yr}` })} style={{ padding: '10px 16px', fontSize: 13, fontWeight: 700, textAlign: 'right', cursor: 'pointer',
              ...
              <tbody>
                {reportBudgetCategories.map(cat => {
                  ...
                  const actual = cat === 'Income'
                    ...
                    : (hasActuals ? (normalizedActuals[cat] || 0) / actualMonthCount : scheduledActual)
                  ...
                  return (
                    <tr key={cat} onClick={() => onOpenTransactions?.({ budgetCategory: cat, direction: cat === 'Income' ? 'income' : 'expense', label: cat })} style={{ borderTop: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer' }}>
...
function CalendarView({ proj, calYear, calMonth, setCalYear, setCalMonth, selDay, setSelDay, accounts, viewAcctIds, onSave, onApplyScopedChange, onMove, onDelete, showActuals, toggleActuals, actualsLoading, actualsError, actualsFreshnessMessage, actualsFreshnessStatus, balanceDataMessage, balanceDataStatus, actualsByDate, plaidActuals, historicalBals = {}, excludedAccountCount = 0, unmappedTransactionCount = 0, balanceSource = 'stored account balances', balanceVerifiedLive = false, todayPlanUnresolved = false, onActualTxClick, readOnly = false }) {
  ...
  const monthBankTransactions = useMemo(
    () => transactionsForCalendarMonth(plaidActuals || [], calYear, calMonth),
    [plaidActuals, calYear, calMonth],
  )
  const monthBankCount = monthBankTransactions.length
  ...
  const prevMonth = () => { let m = calMonth - 1, y = calYear; if (m < 0) { m = 11; y-- }; setSelDay(null); setCalMonth(m); setCalYear(y) }
  const nextMonth = () => { let m = calMonth + 1, y = calYear; if (m > 11) { m = 0; y++ }; setSelDay(null); setCalMonth(m); setCalYear(y) }
  ...
  const handleSave = async tx => {
    ...
    if (await onSave(tx, { occurrenceDate:selTx?._occurrenceDate || selTx?.start, occurrence:Boolean(selTx?._occurrenceDate) })) setSelTx(null)
  }
  const handleDelete = async tx => {
    ...
    if (await onDelete(tx, { occurrenceDate:tx._occurrenceDate || tx.start, occurrence:Boolean(tx._occurrenceDate) })) setSelTx(null)
  }

  const applyScope = async scope => {
    if (readOnly) return false
    if (await onApplyScopedChange({ ...pendingScope, scope })) {
      setPendingScope(null)
      setSelTx(null)
    }
  }
  ...
  // ── Move handlers ────────────────────────────────────────────────────────
  const handleMoveOne = async ({ tx, fromDate, toDate }) => {
    if (readOnly) return false
    if (await onMove({ tx, fromDate, toDate, scope:'one' })) setPendingMove(null)
  }

  const handleMoveSeries = async ({ tx, fromDate, toDate }) => {
    if (readOnly) return false
    if (await onMove({ tx, fromDate, toDate, scope:'future' })) setPendingMove(null)
  }
    ...
    <div>
      ...
      {!readOnly && selTx && (
        <div className="finance-calendar-editor-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.68)', zIndex: 1100,
          ...
          <div className="finance-calendar-editor-dialog" onClick={e => e.stopPropagation()}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              ...
              <button aria-label="Close transaction editor" onClick={() => setSelTx(null)}
      ...
      {!readOnly && pendingMove && (
        <div className="finance-calendar-move-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          onClick={() => setPendingMove(null)}>
          <div className="finance-calendar-move-dialog" onClick={e => e.stopPropagation()}
            style={{ background: 'var(--card-bg, #1a1a1a)', border: '1px solid rgba(197,164,109,0.3)', borderRadius: 18, padding: '28px 28px 24px', maxWidth: 380, width: '90%', boxShadow: '0 24px 64px rgba(0,0,0,0.6)' }}>
            <p style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 600 }}>Move "{pendingMove.tx.name}"</p>
            <p style={{ margin: '0 0 22px', fontSize: 13, color: 'var(--muted)' }}>
              {fmtDateLabel(pendingMove.fromDate)} → {fmtDateLabel(pendingMove.toDate)}
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <button onClick={() => handleMoveOne(pendingMove)}
                style={{ padding: '11px 16px', borderRadius: 10, border: '1px solid rgba(197,164,109,0.35)', background: 'rgba(197,164,109,0.10)', color: 'var(--gold)', cursor: 'pointer', fontSize: 14, fontWeight: 500, textAlign: 'left' }}>
      ...
      <CashForecastIntro
        ...
        freshnessStatus={actualsFreshnessStatus}
        balanceMessage={balanceDataMessage}
        balanceStatus={balanceDataStatus}
      ...
      <div className="finance-calendar-header" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        <button aria-label="Previous financial calendar month" onClick={prevMonth} style={{ padding: '6px 10px', cursor: 'pointer', borderRadius: 8, border: '1px solid var(--glass-border)', background: 'transparent' }}>
          <i className="ti ti-chevron-left" aria-hidden="true" />
        </button>
        <p style={{ margin: 0, fontSize: 15, fontWeight: 600, flex: 1, textAlign: 'center' }}>{monthName}</p>
        <button aria-label="Next financial calendar month" onClick={nextMonth} style={{ padding: '6px 10px', cursor: 'pointer', borderRadius: 8, border: '1px solid var(--glass-border)', background: 'transparent' }}>
          <i className="ti ti-chevron-right" aria-hidden="true" />
        </button>
        <button onClick={() => { const householdToday = getHouseholdCalendarDate(); setSelDay(null); setCalMonth(householdToday.getMonth()); setCalYear(householdToday.getFullYear()) }}
      ...
      <CashForecastAgenda
        ...
        todayKey={todayStr}
        selectedDay={selDay}
        historicalBalances={historicalBals}
        monthName={monthName}
        balanceSource={balanceSource}
        balanceVerifiedLive={balanceVerifiedLive}
        todayPlanUnresolved={todayPlanUnresolved}
      ...
      {/* ── Calendar cells ── */}
      <div className="cal-grid finance-calendar-month-grid">
        {cells.map((cell, i) => {
          ...
          return (
            <div key={key}
              ...
              onDragOver={e => { if (readOnly) return; e.preventDefault(); e.dataTransfer.dropEffect = 'move'; setDragOver(key) }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: showActualPills ? 0 : 1 }}>
                  {sortAndFilterTransactions(pt.txns).slice(0, maxBudget).map((tx, j) => (
                    <div className="finance-calendar-event" key={j}
                      ...
                      onClick={e => { e.stopPropagation(); if (!readOnly) setSelTx({ ...tx, start:key, _occurrenceDate:key, _reviewVersion:captureFinanceReviewVersion() }) }}
                      style={{
              ...
              {showActualPills && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, marginTop: hasTxns ? 2 : 0 }}>
                  ...
                  {dayActuals.length > 3 && (
                    <div style={{ fontSize: 9, color: '#90AADE', lineHeight: '13px', paddingLeft: 2 }}>+{dayActuals.length - 3} actual</div>
                  )}
                </div>
              )}
              {/* Reconstructed history takes priority for past dates. Future
                  dates remain read-only projections from the latest balance. */}
              {(pt || reconstructedBal !== undefined) && (
                <div style={{ marginTop: 'auto', paddingTop: 4, borderTop: '1px solid rgba(255,255,255,0.06)', textAlign: 'right' }}>
                  {reconstructedBal !== undefined ? (
                    <span title="Reconstructed end-of-day balance from the latest bank balance and transaction history"
                      style={{ fontSize: 9, fontWeight: 700, color: reconstructedBal < 0 ? 'var(--expense-color)' : '#90AADE' }}>
      ...
      {selDay && selPt && (
        <div className="finance-card" style={{ marginTop: 16 }}>
          <div className="finance-calendar-day-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>
                {new Date(selDay + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
              <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--muted)' }}>
                {selPt.delta !== 0 ? `Net: ${selPt.delta > 0 ? '+' : ''}${fmtMoney(selPt.delta)}` : 'No budgeted transactions'}
                {selPt.txns.length > 0 && !readOnly ? ' · click to edit · drag to move' : ''}
              </p>
            </div>
            <div style={{ textAlign: 'right' }}>
              {(() => {
                const isSelPast    = selDay < todayStr
                const isSelToday   = selDay === todayStr
                const selReconstructedBal = isSelPast && historicalBals[selDay] !== undefined
                  ? historicalBals[selDay] : undefined
                // Reconstructed history takes priority; fall back to the
                // backward-anchored projection estimate when history is unavailable.
                const displayBal = selReconstructedBal !== undefined ? selReconstructedBal : selPt.bal
                const balColor   = displayBal < 0 ? 'var(--expense-color)' : selReconstructedBal !== undefined ? '#90AADE' : 'var(--gold)'
                const balLabel   = selReconstructedBal !== undefined
                  ...
                  : isSelToday
                    ...
                    : isSelPast
                      ...
                      : todayPlanUnresolved
                        ? 'Projected cash balance · today unresolved'
                        : 'Projected cash balance'
                return (
                  <>
                    <p style={{ margin: 0, fontSize: 11, color: selReconstructedBal !== undefined ? '#90AADE' : 'var(--muted)' }}>
                      {balLabel}
                    </p>
                    <p title={selReconstructedBal !== undefined ? 'Calculated backward from the latest source-managed balance and bank transaction history' : 'Read-only forecast; manual balance overrides are disabled'}
                      style={{ margin: 0, fontSize: 20, fontWeight: 600, color: balColor,
                        opacity: isSelPast && selReconstructedBal === undefined ? 0.85 : 1 }}>
                      {fmtMoney(displayBal)}
                    </p>
                  </>
                )
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {selectedPlannedTransactions.map((tx, idx2) => (
                  <div key={idx2}
                    onClick={e => { e.stopPropagation(); if (!readOnly) setSelTx({ ...tx, start:selDay, _occurrenceDate:selDay, _reviewVersion:captureFinanceReviewVersion() }) }}
                    style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '9px 12px',
                      background: 'rgba(255,255,255,0.04)', borderRadius: 10, cursor: readOnly ? 'default' : 'pointer',
                      border: '1px solid transparent', transition: 'all .15s' }}
                    onMouseEnter={e => { if (!readOnly) { e.currentTarget.style.background = 'rgba(255,255,255,0.08)'; e.currentTarget.style.borderColor = 'rgba(197,164,109,0.22)' } }}
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      <p style={{ margin: '2px 0 0', fontSize: 11, color: 'var(--muted)' }}>
                        ...
                        {!readOnly && <span style={{ color: 'rgba(197,164,109,0.6)', marginLeft: 6 }}>Edit ›</span>}
                      </p>
                    </div>
          ...
          {showActuals && selDay <= todayStr && (() => {
            ...
            const actualActivity = summarizeActualCashActivity(dayActs)
            const actualExp = actualActivity.expenses
            const actualInc = actualActivity.inflows
              ...
              <>
                <div style={{ marginTop: hasPlanned ? 14 : 0 }}>
                  <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 600, color: '#90AADE', letterSpacing: '.08em', textTransform: 'uppercase' }}>
                    {hasActuals ? `Bank activity${dayActs.some(t => t.pending) ? ' — includes pending' : ' — posted'}` : 'Bank activity'}
                  </p>
                  {!hasActuals && (
                    <p style={{ margin: 0, fontSize: 13, color: 'var(--muted)', fontStyle: 'italic' }}>
                      {actualsError ? `Sync error — ${actualsError}` : plaidActuals?.length ? 'No posted transactions on this date.' : plaidActuals !== null ? 'No transactions returned from the existing Plaid connection.' : 'No connected bank source is available for posted actuals; connection changes are disabled in this release.'}
                    </p>
                  )}
                  {hasActuals && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {selectedActualTransactions.map((tx, idx3) => {
                        ...
                        return (
                          <div key={idx3}
                            onClick={e => { e.stopPropagation(); if (!readOnly) onActualTxClick?.(tx) }}
                            style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '9px 12px',
                              background: 'rgba(100,140,220,0.06)', borderRadius: 10, cursor: readOnly ? 'default' : 'pointer',
                              border: `1px solid ${tx.pending ? 'rgba(197,164,109,0.22)' : 'rgba(100,140,220,0.18)'}`,
                              transition: 'all .15s' }}
                            onMouseEnter={e => { if (!readOnly) { e.currentTarget.style.background = 'rgba(100,140,220,0.13)'; e.currentTarget.style.borderColor = 'rgba(100,140,220,0.4)' } }}
                            ...
                            <div style={{ minWidth: 0, flex: 1 }}>
                              ...
                              <p style={{ margin: '2px 0 0', fontSize: 11, color: 'var(--muted)' }}>
                                ...
                                {isTransfer ? ' · Transfer · included in bank balance movement, excluded from income and expense totals' : ''}
                                {tx.category ? ` · ${tx.category}` : ''}
                                {!readOnly && <span style={{ color: 'rgba(144,170,222,0.7)', marginLeft: 6 }}>Edit ›</span>}
                              </p>
                            </div>
                            <p style={{ margin: '0 0 0 12px', fontSize: 13, fontWeight: 600, flexShrink: 0,
                              color: isIncome ? '#7DCBA4' : '#90AADE' }}>
                      ...
                      {selectedActualTransactions.length===0&&<p style={{ margin: 0, fontSize: 12, color: 'var(--muted)' }}>No bank transactions match these filters.</p>}
                    </div>
                  )}
                </div>
                {hasActuals && (hasPlanned || hasActuals) && (
                  <div style={{ marginTop: 12, padding: '10px 14px', background: 'rgba(144,170,222,0.07)', borderRadius: 10, border: '1px solid rgba(144,170,222,0.18)' }}>
                    ...
                    <div className="finance-day-reconciliation-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                      {[
                        { label: 'Planned', exp: plannedExp, inc: plannedInc },
                        { label: 'Bank activity', exp: actualExp, inc: actualInc },
                    ...
                    <p style={{ margin:'8px 0 0', color:'var(--muted)', fontSize:10, textAlign:'center' }}>
                      Bank inflows include {fmtMoney(actualActivity.income)} realized income{actualActivity.otherInflows ? `, ${fmtMoney(actualActivity.otherInflows)} posted refunds or other credits` : ''}{actualActivity.pendingInflows ? `, and ${fmtMoney(actualActivity.pendingInflows)} pending inflows not yet realized` : ''}. {actualActivity.pendingExpenses ? `${fmtMoney(actualActivity.pendingExpenses)} of the outflow is pending. ` : ''}Transfers are excluded.
                    </p>
                  </div>
                )}
              </>
          ...
          {selPt.txns.length === 0 && selDay <= todayStr && (!showActuals || !actualsByDate?.[selDay]?.length) && (
            <p style={{ margin: '14px 0 0', fontSize: 13, color: 'var(--muted)' }}>No planned items on this date.</p>
          )}

          {/* Add transaction to this date */}
          {!readOnly && <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <button
              onClick={e => { e.stopPropagation(); setSelTx({ name: '', amount: '', type: 'expense', freq: 'once', start: selDay, end: '', cat: 'Housing', acct: accounts[0]?.id || '', transferTo: '', _reviewVersion:captureFinanceReviewVersion() }) }}
