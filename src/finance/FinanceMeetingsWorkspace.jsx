...
export default function FinanceMeetingsWorkspace({liveSnapshot={},drilldowns={},accountScope='Selected accounts'}){
  ...
  return <section className={`finance-meetings${isRecording?' is-recording':''}`}>
    ...
    <section className="fm-workbench"><div className="fm-panel"><div className="fm-panel-title"><div><span>Decisions & Assignments</span><small>Assignments publish to the Family Calendar automatically</small></div><i className="ti ti-list-check"/></div><div className="fm-form-grid"><input value={decision.text} onChange={event=>setDecision(value=>({...value,text:event.target.value}))} placeholder="What did we decide?"/><input value={decision.owner} onChange={event=>setDecision(value=>({...value,owner:event.target.value}))} placeholder="Owner (optional — Family if blank)"/><input type="date" aria-label="Assignment due date" value={decision.due} onChange={event=>setDecision(value=>({...value,due:event.target.value}))}/><button onClick={addDecision}>Add to Calendar</button></div><div className="fm-action-list">{(workspace.openActions||[]).length?(workspace.openActions||[]).map(item=><Action key={item.id} item={item} onToggle={toggleAction}/>):<p className="fm-empty">No open commitments.</p>}</div></div>
      <div className="fm-panel"><div className="fm-panel-title"><div><span>Proposed Financial Corrections</span><small>Review proposals here. Approval records the decision; it does not alter bank data.</small></div><i className="ti ti-adjustments"/></div><div className="fm-correction-grid"><input value={correction.label} onChange={event=>setCorrection(value=>({...value,label:event.target.value}))} placeholder="What changed?"/><input value={correction.value} onChange={event=>setCorrection(value=>({...value,value:event.target.value}))} placeholder="Correct value"/><select value={correction.source} onChange={event=>setCorrection(value=>({...value,source:event.target.value}))}>{SOURCE_STATES.map(source=><option key={source}>{source}</option>)}</select><select value={correction.scope} onChange={event=>setCorrection(value=>({...value,scope:event.target.value}))}><option>this occurrence</option><option>going forward</option><option>underlying data is wrong</option></select><button onClick={addCorrection}>Stage correction</button></div>{(workspace.corrections||[]).slice(0,8).map(item=>{const legacy=!item.origin;return <article className={`fm-correction-row${legacy?' is-unverified':''}`} key={item.id}><div><strong>{item.label}: {item.value}</strong><span>{legacy?'Unverified legacy entry · source detail was not retained':`${item.source} · ${item.scope} · ${item.origin} · ${item.meetingDate||new Date(item.createdAt).toLocaleDateString()}`}</span>{item.reason&&<small>{item.reason}</small>}</div><div className="fm-correction-actions"><button type="button" onClick={()=>dismissCorrection(item.id)}>Dismiss</button><button disabled={legacy||item.status==='approved'} title={legacy?'Legacy entries without provenance cannot be approved.':''} onClick={()=>approveCorrection(item.id)}>{legacy?'Needs source':item.status==='approved'?'Approved':'Approve'}</button></div></article>})}</div></section>
