...
export default function FinanceMeetingsWorkspace({liveSnapshot={},drilldowns={},accountScope='Selected accounts',currentMember='Household member',readOnly=false,financeReadOnly=readOnly,meetingPlanningReadOnly=readOnly}){
  ...
  const analyze=async()=>{if(meetingPlanningReadOnly||!transcript.trim())return;setAiBusy(true);setAiError('');try{
    const result=await analyzeMeetingTranscript({transcript,cadence,snapshot,notes})
