...
function CorrectionRow({item,onSave,onDismiss,onApprove}){
  const[editing,setEditing]=useState(false),[draft,setDraft]=useState(item),legacy=!item.origin
  useEffect(()=>setDraft(item),[item])
  if(editing)return <article className={`fm-correction-row fm-edit-row${legacy?' is-unverified':''}`}><div className="fm-inline-editor"><input aria-label="Correction name" value={draft.label} onChange={event=>setDraft(value=>({...value,label:event.target.value}))}/><input aria-label="Correction value" value={draft.value} onChange={event=>setDraft(value=>({...value,value:event.target.value}))}/><textarea aria-label="Correction rationale" value={draft.reason||''} onChange={event=>setDraft(value=>({...value,reason:event.target.value}))} placeholder="Why is this correction needed?"/><select aria-label="Correction source" value={draft.source} onChange={event=>setDraft(value=>({...value,source:event.target.value}))}>{SOURCE_STATES.map(source=><option key={source}>{source}</option>)}</select><select aria-label="Correction scope" value={draft.scope} onChange={event=>setDraft(value=>({...value,scope:event.target.value}))}><option>this occurrence</option><option>going forward</option><option>underlying data is wrong</option></select><div><button type="button" onClick={()=>{setDraft(item);setEditing(false)}}>Cancel</button><button type="button" className="fm-save" onClick={()=>{onSave(item.id,draft);setEditing(false)}}>Save</button></div></div></article>
  return <article className={`fm-correction-row${legacy?' is-unverified':''}`}><div><strong>{item.label}: {item.value}</strong><span>{legacy?'Unverified legacy entry · source detail was not retained':`${item.source} · ${item.scope} · ${item.origin} · ${item.meetingDate||new Date(item.createdAt).toLocaleDateString()}`}</span>{item.reason&&<small>{item.reason}</small>}{item.updatedBy&&<small>Edited by {item.updatedBy} · {new Date(item.updatedAt).toLocaleString()}</small>}</div><div className="fm-correction-actions"><button type="button" className="fm-edit" onClick={()=>setEditing(true)}><i className="ti ti-edit"/> Edit</button><button type="button" onClick={()=>onDismiss(item.id)}>Dismiss</button><button disabled={legacy||item.status==='approved'} title={legacy?'Legacy entries without provenance cannot be approved.':''} onClick={()=>onApprove(item.id)}>{legacy?'Needs source':item.status==='approved'?'Approved':'Approve'}</button></div></article>
}

function HistoryRow({item,onSave}){
  const[editing,setEditing]=useState(false),[draft,setDraft]=useState(item)
  useEffect(()=>setDraft(item),[item])
  if(editing)return <article className="fm-history-edit"><div className="fm-inline-editor"><textarea aria-label="Meeting summary" value={draft.summary||''} onChange={event=>setDraft(value=>({...value,summary:event.target.value}))} placeholder="Meeting summary"/><textarea aria-label="Meeting notes" value={draft.notes||''} onChange={event=>setDraft(value=>({...value,notes:event.target.value}))} placeholder="Meeting notes"/><textarea aria-label="Meeting transcript" value={draft.transcript||''} onChange={event=>setDraft(value=>({...value,transcript:event.target.value}))} placeholder="Meeting transcript"/><div><button type="button" onClick={()=>{setDraft(item);setEditing(false)}}>Cancel</button><button type="button" className="fm-save" onClick={()=>{onSave(item.id,draft);setEditing(false)}}>Save</button></div></div></article>
  return <article><div><strong>{LABELS[item.cadence]} Finance Meeting</strong><span>{new Date(item.endedAt).toLocaleString()}{item.updatedBy?` · edited by ${item.updatedBy}`:''}</span></div><small>{item.summary||item.notes||'Meeting captured and reconciled.'}</small><button type="button" className="fm-edit" onClick={()=>setEditing(true)}><i className="ti ti-edit"/> Edit</button></article>
}

export default function FinanceMeetingsWorkspace({liveSnapshot={},drilldowns={},accountScope='Selected accounts',currentMember='Household member'}){
  ...
  const stageCorrection=(item,{origin='meeting analysis'}={})=>{if(!item?.label||item.value==='')return;setWorkspace(current=>({...current,corrections:[{id:id('correction'),label:item.label,value:item.value,source:item.source||'Proposed',scope:item.scope||'this occurrence',reason:item.reason||'',origin,meetingDate:meetingDate(),status:'proposed',createdAt:new Date().toISOString(),cadence},...(current.corrections||[])]}))}
  const addCorrection=()=>{if(!correction.label.trim()||correction.value==='')return;stageCorrection(correction,{origin:'manually entered'});setCorrection({label:'',value:'',source:'User Confirmed',scope:'this occurrence'})}
  ...
  const dismissCorrection=correctionId=>setWorkspace(current=>({...current,corrections:(current.corrections||[]).filter(item=>item.id!==correctionId)}))
  const updateCorrection=(correctionId,changes)=>setWorkspace(current=>({...current,corrections:(current.corrections||[]).map(item=>item.id===correctionId?{...item,label:String(changes.label||'').trim(),value:changes.value,reason:String(changes.reason||'').trim(),source:changes.source,scope:changes.scope,status:'proposed',updatedAt:new Date().toISOString(),updatedBy:currentMember}:item)}))
  const updateMeeting=(meetingId,changes)=>setWorkspace(current=>({...current,meetings:(current.meetings||[]).map(item=>item.id===meetingId?{...item,summary:changes.summary||'',notes:changes.notes||'',transcript:changes.transcript||'',updatedAt:new Date().toISOString(),updatedBy:currentMember}:item)}))

  const analyze=async()=>{if(!transcript.trim())return;setAiBusy(true);setAiError('');try{const result=await analyzeMeetingTranscript({transcript,cadence,snapshot,notes});setAiSummary(result.summary||'');(result.actions||[]).forEach(item=>addAction(item));(result.decisions||[]).forEach(item=>addAction({text:item.text,owner:'',due:'',financialEffect:''}));(result.corrections||[]).forEach(item=>stageCorrection(item,{origin:'meeting transcript analysis'}));if((result.unresolved||[]).length)setNotes(current=>`${current}${current?'\n\n':''}Unresolved: ${(result.unresolved||[]).join(' · ')}`)}catch(error){setAiError(error.message||'Brevity could not reconcile this meeting.')}finally{setAiBusy(false)}}
  const finalize=()=>{
  ...
  return <section className={`finance-meetings${isRecording?' is-recording':''}`}>
    ...
    <section className="fm-workbench"><div className="fm-panel"><div className="fm-panel-title"><div><span>Decisions & Assignments</span><small>Assignments publish to the Family Calendar automatically</small></div><i className="ti ti-list-check"/></div><div className="fm-form-grid"><input value={decision.text} onChange={event=>setDecision(value=>({...value,text:event.target.value}))} placeholder="What did we decide?"/><input value={decision.owner} onChange={event=>setDecision(value=>({...value,owner:event.target.value}))} placeholder="Owner (optional — Family if blank)"/><input type="date" aria-label="Assignment due date" value={decision.due} onChange={event=>setDecision(value=>({...value,due:event.target.value}))}/><button onClick={addDecision}>Add to Calendar</button></div><div className="fm-action-list">{(workspace.openActions||[]).length?(workspace.openActions||[]).map(item=><Action key={item.id} item={item} onToggle={toggleAction} onSave={updateAction}/>):<p className="fm-empty">No open commitments.</p>}</div></div>
      <div className="fm-panel"><div className="fm-panel-title"><div><span>Proposed Financial Corrections</span><small>Review proposals here. Approval records the decision; it does not alter bank data.</small></div><i className="ti ti-adjustments"/></div><div className="fm-correction-grid"><input value={correction.label} onChange={event=>setCorrection(value=>({...value,label:event.target.value}))} placeholder="What changed?"/><input value={correction.value} onChange={event=>setCorrection(value=>({...value,value:event.target.value}))} placeholder="Correct value"/><select value={correction.source} onChange={event=>setCorrection(value=>({...value,source:event.target.value}))}>{SOURCE_STATES.map(source=><option key={source}>{source}</option>)}</select><select value={correction.scope} onChange={event=>setCorrection(value=>({...value,scope:event.target.value}))}><option>this occurrence</option><option>going forward</option><option>underlying data is wrong</option></select><button onClick={addCorrection}>Stage correction</button></div>{(workspace.corrections||[]).slice(0,8).map(item=><CorrectionRow key={item.id} item={item} onSave={updateCorrection} onDismiss={dismissCorrection} onApprove={approveCorrection}/>)}</div></section>

    <section className="fm-capture"><div className="fm-panel-title"><div><span>Meeting Capture</span><small>Live transcript uses the same microphone stream as the master recording.</small></div><div className="fm-capture-actions"><button onClick={()=>fileRef.current?.click()}><i className="ti ti-file-upload"/> Import Otter</button><input ref={fileRef} type="file" accept=".txt,.vtt,.srt,text/plain,text/vtt" onChange={importTranscript} hidden/>{masterChunks.current.length>0&&<button onClick={saveRecording}><i className="ti ti-download"/> Save recording</button>}<button onClick={analyze} disabled={aiBusy||!transcript.trim()}><i className="ti ti-sparkles"/> {aiBusy?'Analyzing…':'Analyze with Brevity'}</button></div></div>{aiError&&<div className="fm-ai-error">{aiError}</div>}{aiSummary&&<div className="fm-ai-summary"><strong>Brevity summary</strong><textarea aria-label="Brevity meeting summary" value={aiSummary} onChange={event=>setAiSummary(event.target.value)} rows="4"/></div>}<textarea className="fm-transcript" value={transcript} onChange={event=>setTranscript(event.target.value)} rows="9" placeholder="Live transcription appears here. You can also paste or import an Otter transcript."/><textarea className="fm-notes" value={notes} onChange={event=>setNotes(event.target.value)} rows="3" placeholder="Optional meeting notes or unresolved context…"/></section>
    ...
    {(workspace.meetings||[]).length>0&&<section className="fm-history"><div className="fm-panel-title"><div><span>Meeting History</span><small>Institutional memory for the household</small></div></div>{workspace.meetings.slice(0,6).map(item=><HistoryRow key={item.id} item={item} onSave={updateMeeting}/>)}</section>}
