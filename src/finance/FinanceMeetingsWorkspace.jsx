...
export default function FinanceMeetingsWorkspace({liveSnapshot={},drilldowns={},accountScope='Selected accounts'}){
  ...
  const[decision,setDecision]=useState({text:'',owner:'',due:''}),[correction,setCorrection]=useState({label:'',value:'',source:'User Confirmed',scope:'this occurrence'}),[drilldown,setDrilldown]=useState(null)
  ...
  const gapDrilldown={label:'Gap to monthly vision',amount:visionGap,note:'Target monthly net cash flow minus current monthly net cash flow.',source:'Vision target + current finance data',children:[{label:'Vision target',amount:snapshot.goalMonthlyNet},{label:'Current monthly net',amount:snapshot.currentMonthlyNet,children:drilldowns.currentMonthlyNet?.children||[]} ]}
  ...
  return <section className={`finance-meetings${isRecording?' is-recording':''}`}>
    ...
    <div className="fm-kiss-grid">
      {cadence==='daily'?<>
        <article className="fm-kiss-card"><span className="fm-step">01</span><h2>Vision</h2><p>Keep today connected to the $50K monthly net-cash-flow target.</p><button type="button" className="fm-big-number fm-big-number--clickable" onClick={()=>setDrilldown(goalDrilldown)}>{money(snapshot.goalMonthlyNet)}<i className="ti ti-chevron-right"/></button><div className="fm-metrics"><Metric label="Current monthly net" value={money(snapshot.currentMonthlyNet)} onClick={()=>setDrilldown(drilldowns.currentMonthlyNet)}/><Metric label="Gap to vision" value={money(visionGap)} onClick={()=>setDrilldown(gapDrilldown)}/></div></article>
        ...
        <article className="fm-kiss-card"><span className="fm-step">01</span><h2>Vision</h2><p>Target monthly net cash flow</p><button type="button" className="fm-big-number fm-big-number--clickable" onClick={()=>setDrilldown(goalDrilldown)}>{money(snapshot.goalMonthlyNet)}<i className="ti ti-chevron-right"/></button><div className="fm-metrics"><Metric label="Current monthly net cash flow" value={money(snapshot.currentMonthlyNet)} onClick={()=>setDrilldown(drilldowns.currentMonthlyNet)}/></div></article>
